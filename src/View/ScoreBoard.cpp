...
void ScoreBoard::draw_perimeter(const Cairo::RefPtr<Cairo::Context>& cr,
                                const Color& color, size_t param,
...
bool ScoreBoard::on_draw(const Cairo::RefPtr<Cairo::Context>& cr)
{
    ...
    for (auto&& score : scores) {
        ...
        const std::string allInfo = std::to_string(playerPlacement) + " - " +
                                    player->name + " - " +
        ...
        /** Center align */
        cr->move_to((size.width) / 2 - te.x_bearing - te.width / 2,
                    size.height * titleRatio + 40 + 35 * ind - te.y_bearing -
                        te.height / 2);
