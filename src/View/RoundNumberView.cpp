...
RoundNumberView::RoundNumberView(const size_t& _roundNumber,
...
void RoundNumberView::draw_round_digit(const Cairo::RefPtr<Cairo::Context>& cr)
{
    ...
    unsigned int topLeftY =
        upperPartHeight + lowerPartHeight / 2 - 2.5 * double(digitBlockSize);

    if (roundNumber < 10) {
        draw_digit(cr, roundNumber, Point {topLeftX, topLeftY});
    }
    else {
        draw_digit(cr, roundNumber / 10,
                   Point {size_t(topLeftX - 1.5 * digitBlockSize), topLeftY});
        draw_digit(cr, roundNumber % 10,
                   Point {size_t(topLeftX + 1.5 * digitBlockSize), topLeftY});
    }
}

void RoundNumberView::draw_calendar_background(
    const Cairo::RefPtr<Cairo::Context>& cr)
{
    /** Lower rectangle begin */
    cr->move_to(size.width, upperPartHeight);
    cr->arc(x + size.width - radius, size.height - radius, radius, 0 * degrees,
            90 * degrees);
    cr->arc(x + radius, size.height - radius, radius, 90 * degrees,
            180 * degrees);
    ...
    cr->arc(x + size.width - radius, y + radius, radius, -90 * degrees,
            0 * degrees);
    ...
    for (unsigned int i = 0; i < clipCount; ++i) {
        ...
        cr->arc(i * widthForCircle + widthForCircle / 2 + x + margin,
                y + localRadius, 3, 0.0, 2 * M_PI);
        cr->set_source_rgba(1, 1, 1, 1);
        cr->fill();

        /** black clip */
        cr->set_source_rgba(0, 0, 0, 1);
        cr->arc(i * widthForCircle + widthForCircle / 2 + x + margin, y,
                localRadius, M_PI, M_PI / 2);
        cr->stroke();
    }
    /** Clips end */
