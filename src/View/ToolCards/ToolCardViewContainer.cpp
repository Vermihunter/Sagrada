    ...
    : currToolCardView(nullptr)
{
    ...
    for (auto&& gameTc : game.get_ctx()->toolCards) {
        tcCards.push_back(ToolCardView::create(
            *this, game, *gameTc,
            SizeInPixels {cardWidth, size_t(cardWidth / tcWidthToHeightRatio)},
            ind));
...
void ToolCardViewContainer::connect_tool_card(
    ToolCardView* newlySelectedToolCardView)
{
...
bool ToolCardViewContainer::on_common_da_draw(
