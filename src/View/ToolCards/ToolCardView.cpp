...
ToolCardView::ToolCardView(ToolCardViewContainer& _parentContainer, Game& _game,
    ...
    , costSize(selectionSize.width, 22)
    , image(tcImagePaths.at(_tc.get_type()), size)
    , isSelected(false)
    , signalProcessor(nullptr)
{
    da_SelectionArea.signal_draw().connect(
        sigc::mem_fun(*this, &ToolCardView::on_selection_draw));

    da_SelectionArea.set_size_request(selectionSize.width,
...
tc_view_t ToolCardView::create(ToolCardViewContainer& parentContainer,
                               ...
                               const SizeInPixels& size, size_t ind)
{
    ...
    switch (tc.get_type()) {
        case tc_etype::CHANGE_DIE_VALUE:
            result = std::make_unique<ChangeDieValueTCView>(
                parentContainer, game, tc, size, ind);
            break;
        case tc_etype::MOVE_TWICE:
            result = std::make_unique<MoveTwiceInARowTCView>(
                parentContainer, game, tc, size, ind);
            break;
        case tc_etype::DIE_WITHOUT_NEIGHBORS:
            result = std::make_unique<PutDieWithoutNeighborTCView>(
                parentContainer, game, tc, size, ind);
            break;
        case tc_etype::REROLL_ALL_DICE:
            result = std::make_unique<RerollAllDiceTCView>(parentContainer,
                                                           game, tc, size, ind);
            break;
        case tc_etype::RELOCATE_TWO_SAME_COLORED_DICE:
            result = std::make_unique<RelocateMaxTwoSameColorDiceTCView>(
                parentContainer, game, tc, size, ind);
            break;
        case tc_etype::REPLACE_FROM_ROUND_TRACK:
            result = std::make_unique<ReplaceDieFromSupplyToRoundTrackTCView>(
                parentContainer, game, tc, size, ind);
            break;
        case tc_etype::RANDOM_BAG_DIE:
            result = std::make_unique<ReplaceDieWithRandomFromBagTCView>(
                parentContainer, game, tc, size, ind);
            break;
        case tc_etype::REROLL_DIE:
            result = std::make_unique<RerollDieTCView>(parentContainer, game,
                                                       tc, size, ind);
            break;
        case tc_etype::REVERSE_DIE_VALUE:
            result = std::make_unique<ReverseDieValueTCView>(
                parentContainer, game, tc, size, ind);
            break;
        case tc_etype::MOVE_IGNORE_VAL:
            result = std::make_unique<
                RelocateDieIgnoreAttributeTCView<DTFM_MODE_IGNORE_VALUE>>(
                parentContainer, game, tc, size, ind);
            break;
        case tc_etype::MOVE_IGNORE_COLOR:
            result = std::make_unique<
                RelocateDieIgnoreAttributeTCView<DTFM_MODE_IGNORE_COLOR>>(
                parentContainer, game, tc, size, ind);
            break;
        case tc_etype::RELOCATE_TWO_DICE:
            result = std::make_unique<RelocateExactlyTwoDiceTCView>(
                parentContainer, game, tc, size, ind);
            break;
        case tc_etype::COUNT:
            break;
...
void ToolCardView::set_signal_receiver(
    IToolCardViewSignalProcessor* _signalProcessor)
{
 ...
 */
void ToolCardView::on_common_help_visualizer_draw(
...
bool ToolCardView::on_cost_visualizer_draw(
    const Cairo::RefPtr<Cairo::Context>& cr)
{
    ...
    const SizeInPixels textEnd {
        size_t((selectionSize.width / 2) + te.x_bearing + te.width / 2),
    ...
    for (size_t i = 0; i < tc.favor_tokens_to_use(); ++i) {
        cr->arc(textEnd.width + i * (circleRadius + x_distanceBetweenCircles) +
                    8,
                textEnd.height, circleRadius, 0.0, 2 * M_PI);
    }
    cr->fill();
...
move_c ToolCardView::collect_possible_moves(ToolCard& concreteToolCard,
                                            tool_card_move_t prevMove)
{
    move_c tmpMoveContainer;
    ToolCardAppendContext tmpAppendContext {
        tmpMoveContainer, game, ind, game.get_curr_state()->player_on_move_id(),
        prevMove};
