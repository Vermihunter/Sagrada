...
BoardFieldView::BoardFieldView(const Point& _upperLeft,
...
void BoardFieldView::draw(const Cairo::RefPtr<Cairo::Context>& cr,
                          bool drawEmptyFields)
{
    ...
    if (selectedColor) {
        cr->set_line_width(7);
        cr->set_source_rgba(selectedColor->r, selectedColor->g,
                            selectedColor->b, 1);
        cr->stroke_preserve();
    }
                                        ...
                                        Color& circleBackgroundColor)
{
    ...
    switch (val.value()) {
        case value_t::ONE:
            draw_circles_by_value(cr, oneRadiusMultiplier,
                                  circleBackgroundColor);
            break;
        case value_t::TWO:
            draw_circles_by_value(cr, twoRadiusMultiplier,
                                  circleBackgroundColor);
            break;
        case value_t::THREE:
            draw_circles_by_value(cr, threeRadiusMultiplier,
                                  circleBackgroundColor);
            break;
        case value_t::FOUR:
            draw_circles_by_value(cr, fourRadiusMultiplier,
                                  circleBackgroundColor);
            break;
        case value_t::FIVE:
            draw_circles_by_value(cr, fiveRadiusMultiplier,
                                  circleBackgroundColor);
            break;
        case value_t::SIX:
            draw_circles_by_value(cr, sixRadiusMultiplier,
                                  circleBackgroundColor);
            break;
        case value_t::COUNT:
        default:
    ...
    Color& circleBackgroundColor)
{
    ...
    for (auto&& [xCordRadiusMultiplier, yCordRadiusMultiplier] :
         radiusMultiplier) {
        draw_circle(cr,
                    middle.x + xCordRadiusMultiplier *
                                   rectangleValueCircleRadiusInPixels,
                    middle.y + yCordRadiusMultiplier *
                                   rectangleValueCircleRadiusInPixels,
                    rectangleValueCircleRadiusInPixels, circleBackgroundColor);
    }
}

void BoardFieldView::draw_circle(const Cairo::RefPtr<Cairo::Context>& cr,
                                 size_t centerXInPixels, size_t centerYInPixels,
                                 size_t radiusInPixels, Color& fill_color)
{
    ...
    cr->set_source_rgb(double(fill_color.r), double(fill_color.g),
                       double(fill_color.b));
    cr->fill();
...
Color& BoardFieldView::select_circle_background_color(
...
// One
std::array<std::pair<int, int>, 1> BoardFieldView::oneRadiusMultiplier = {
    {{0, 0}}};

// Two
std::array<std::pair<int, int>, 2> BoardFieldView::twoRadiusMultiplier = {
    {{2, 2}, {-2, -2}}};
