...
RoundTrackView::RoundTrackView(const SizeInPixels&      fieldSize,
    ...
    : firstDieIndex(0), roundTrackDice(_roundTrackDice),
      displayedBoardFieldCount(_displayedBoardFieldCount)
{
	...
	size.width =
	    displayedBoardFieldCount * (fieldSize.width + segmentSize) + 2 * radius + segmentSize;
	...
	for (size_t i = 0; i < displayedBoardFieldCount; ++i) {
		...
		roundTrackFields.push_back(std::make_unique<BoardFieldView>(
		    Point{.x = size_t(radius + segmentSize + i * (fieldSize.width + segmentSize)),
		          .y = segmentSize},
...
bool RoundTrackView::on_draw(const Cairo::RefPtr<Cairo::Context>& cr)
{
	...
	cr->arc(size.width - radius, size.height - radius, radius, 0 * degrees, 90 * degrees);
...
{
	std::cout << "RoundTrackView::on_button_press_event: [" << event->x << "," << event->y << "]"
	          << std::endl;
	auto x = event->x;
	...
	/** field hit */
		...
		for (auto&& field : roundTrackFields) {
			...
			if (fieldDie && field->contains_point(Point{size_t(event->x), size_t(event->y)})) {
				signal_all(fieldDie);
				break;
			}
		}
