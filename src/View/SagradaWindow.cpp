...
SagradaWindow::SagradaWindow(const SizeInPixels& _size, game_mode_t gameMode,
    ...
    : homePage(ViewContext {_size, Point {0, 0},
                            *this} /*, gameMode, std::move(tournamentConfig)*/)
    , sagradaLogo(v_constants::sagradaLogo::path,
                  v_constants::sagradaLogo::size)
    , size(_size)
{
    set_size_request(size.width, size.height);

    fixed_Logo.set_size_request(size.width, 100);
    fixed_Logo.put(sagradaLogo,
                   (size.width - v_constants::sagradaLogo::size.width) / 2, 10);
...
void SagradaWindow::show_home_page() { show_page(homePage, true); }
...
{
    if (!customConfigPage) {
        customConfigPage = std::make_unique<CustomConfigPage>(
            ViewContext {size, Point {0, 0}, *this});
...
void SagradaWindow::remove_page(Page& page) { fixed.remove(page); }