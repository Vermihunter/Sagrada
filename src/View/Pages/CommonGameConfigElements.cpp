...
CommonGameConfigElements::CommonGameConfigElements(const ViewContext& _ctx,
    ...
    : ctx(_ctx), lne_Seed("Enter a random seed for the " + pageName), button_Start("Start"),
      button_Back("Back"), label_SelectAIPlayers("Select AI players"),
      chb_GameDeterministic("Deterministic game"),
      gameCtxWidgets(ctx, GameCache::get().get_defined_game_contexts(), _localPlayerCount)

{
	...
	fixed.put(vbox_AllWidgets, (ctx.size.width - 400) / 2, ctx.size.height / 2 - 67);

	size_t selectedPlayerCount = std::stoul(gameCtxWidgets.get_player_count_cb().get_active_text());
...
game_config_t CommonGameConfigElements::get_selected_information(bool ignoreNotEnteredSeed)
{
	auto commonGameConfigInfo = std::make_unique<GameConfig>();
	try {
		commonGameConfigInfo->gameCtxBuilder = gameCtxWidgets.get_context_builder();
	}
	catch (const std::exception& e) {
		ctx.mainWindow.display_error_message("Invalid configuration!", e.what());
		return nullptr;
	}

	try {
		commonGameConfigInfo->randomSeed = lne_Seed.widget.get_underlying_number("Seed");
	}
	catch (const std::exception& e) {
	...
	commonGameConfigInfo->gameDeterministic = chb_GameDeterministic.get_active();
...
{
	int  selectedPlayerCount = std::stoi(gameCtxWidgets.get_player_count_cb().get_active_text());
	int  extraPlayerCount    = std::abs(int(AI_playerConfigBoxes.size()) - selectedPlayerCount);
...
CommonGameConfigElements::get_selected_type_specifier(size_t ind)
{
	...
	case 0:
		result = std::make_unique<SimpleAI_TypeSpecifierView<RandomPlayerConfig>>();
		break;
	case 1:
		result = std::make_unique<SimpleAI_TypeSpecifierView<FirstPlayerConfig>>();
		break;
	case 2:
		/* code */
...
void CommonGameConfigElements::add_new_ai_config_box()
{
	...
	lastAdded.signal_changed().connect(
	    sigc::bind(sigc::mem_fun(*this, &CommonGameConfigElements::ai_type_changed),
...
{
	auto& lastAIConfigButton = AI_playerConfigBoxes[AI_playerConfigBoxes.size() - 1];
