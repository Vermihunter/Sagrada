...
GamePlayingPageContext::GamePlayingPageContext(
    ...
    : roundTrackView(SizeInPixels{50, 50}, controller.get_game().get_ctx()->roundTrack.get_dice(),
                     8),
      currRoundsDice(SizeInPixels{50, 50},
      ...
      commonGameElements(std::move(_commonGameElements)), button_Quit("Quit"),
      ...
      ctx(_ctx)
{
	...
	for (auto&& player : controller.get_game().get_ctx()->players) {
		auto playerWpc = std::make_unique<RectangularBoardView>(*player->board->get_ctx(), wpcSize,
		                                                        player.get()); //
		size_t extraTopSpace = (ctx.size.height - (2 * playerWpc->get_size().height)) / 2;
	...
	puocIcon =
	    CardIcon::create_puoc_icon(iconSizes, commonGameElements->puocViews, currentlyShownCards);
	tcIcon = CardIcon::create_tc_icon(iconSizes, commonGameElements->tcViews, currentlyShownCards);
	procIcon =
	    CardIcon::create_proc_icon(iconSizes, commonGameElements->procViews, currentlyShownCards);
	...
	if (roundTrackSize.width > diceSize.width) {
		currRoundsDice.set_margin_left((roundTrackSize.width - diceSize.width) / 2);
	}
	else {
		roundTrackView.set_margin_left((diceSize.width - roundTrackSize.width) / 2);
	}

	size_t cardIconY = 105 + roundTrackSize.height + diceSize.height + 40 + 10;

	size_t longestWidth =
	    std::max(roundTrackView.get_size().width, currRoundsDice.get_size().width);
	...
	fixed.put(commonGameElements->puocViews, (ctx.size.width - (3 * (155 + 10))) / 2,
	          cardIconY + 50 + 10);
	fixed.put(commonGameElements->tcViews, (ctx.size.width - (3 * (155 + 10))) / 2,
	          cardIconY + 50 + 10);
	fixed.put(commonGameElements->procViews, (ctx.size.width - (155 + 10)) / 2,
	          cardIconY + 50 + 10);
	fixed.put(roundNumberView, (ctx.size.width - v_constants::roundNumberSize.width) / 2, 0);
}

void GamePlayingPageContext::construct_scoreboard(eval_state_c&& scores)
{
	scoreBoard = std::make_unique<ScoreBoard>(SizeInPixels{400, 260}, std::move(scores));
