...
SimulationGamePlayingPage::SimulationGamePlayingPage(Page& _prevPage,
                                                     bool  _showSagradaLogoOnQuitButton,
                                                     const ViewContext&   ctx,
                                                     only_ai_controller_t _controller, game_t _game)
    : GamePlayingPage(ctx, *_controller), prevPage(_prevPage),
      showSagradaLogoOnQuitButton(_showSagradaLogoOnQuitButton), pageCtx(ctx, *_controller),
      game(std::move(_game)), controller(std::move(_controller))
{
	initialize(pageCtx);

	pageCtx.button_Next.signal_clicked().connect(
	    sigc::mem_fun(*this, &SimulationGamePlayingPage::next_move_button_clicked));
...
void SimulationGamePlayingPage::next_move_button_clicked()
{
	...
	else if (prevRoundNumber != controller->get_game().get_curr_state()->roundNumber) {
