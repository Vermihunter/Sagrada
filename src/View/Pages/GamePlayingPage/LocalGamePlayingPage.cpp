...
LocalGamePlayingPage::LocalGamePlayingPage(const ViewContext&                  ctx,
    ...
    : GamePlayingPage(ctx, *_controller),
      ...
      currState(pageCtx, defaultState), controller(std::move(_controller)), game(std::move(_game))
{
	initialize(pageCtx);
...
{
	std::cout << "LocalGamePlayingPage::receive_round_track_die_click_signal()" << std::endl;
...
void LocalGamePlayingPage::receive_die_click_signal(Die* die)
{
	...
	if (!controller->local_player_on_move()) {
		display_error_message("Invalid click", " You can select a die only in your turn!");
		return;
	}
	...
	if (currState.selectedTc) {
		...
		highlight_basic_moves_for_die(die, game->get_curr_state()->player_on_move_id());
	}
}

void LocalGamePlayingPage::receive_wpc_click_signal(RectangularBoardView*) {}

void LocalGamePlayingPage::receive_board_field_click_signal(RectangularBoardView* board,
                                                            const BoardIndex&     ind)
{
	...
	if (currState.selectedTc) {
		return currState.selectedTc->receive_board_field_click_signal(board, ind);
	}

	Die* selectedDie = currState.selectedDie;
	if (!selectedDie) {
		return display_error_message("No selected die!", "Select a die to place");
	}

	currState.unselect_selected_die();

	const bool basicMoveSucceeded =
	    send_move_request(DieToFieldMove::create(0, selectedDie, ind), dtfmResultHandlingInfo);
...
void LocalGamePlayingPage::block_features(const BlockedUserInterraction& newState)
{
...
void LocalGamePlayingPage::tool_card_clicked(ToolCardView* clickedTcView, bool forceSelect)
{
	if (!forceSelect && currState.blockState.blockedTcClick) {
		return;
	}

	ID_t playerOnMoveID = controller->get_game().get_curr_state()->player_on_move_id();
	if (!forceSelect && !controller->is_local_player_id(playerOnMoveID)) {
		ctx.mainWindow.display_error_message("It's not your turn!",
		                                     "You can use tool cards only in your turn!");
		return;
	}
...
bool LocalGamePlayingPage::send_move_request(move_t                          m,
                                             const MoveResultHandlingPolicy& resultHandlingInfo)
{
...
void LocalGamePlayingPage::highlight_basic_moves_for_die(Die* die, ID_t playerID,
                                                         dtfm_mode_mask_t mode)
{
	highlight_fields(
	    game->get_ctx()->basicMoveHandler->indices_to_put_to(*game, playerID, die, mode), playerID);
}

void LocalGamePlayingPage::highlight_fields(const board_ind_c& indices, ID_t playerID)
{
...
void LocalGamePlayingPage::pass_move_button_clicked()
{
	...
	send_move_request(
	    PassMove::create(controller->get_game().get_curr_state()->player_on_move_id()),
...
bool LocalGamePlayingPage::on_timeout()
{
	...
	size_t prevRoundNumber = controller->get_game().get_curr_state()->roundNumber;
	...
	if (prevRoundNumber != controller->get_game().get_curr_state()->roundNumber) {
		signal_new_round_start();
	}
...
{
	return **std::find_if(pageCtx.playerBoards.begin(), pageCtx.playerBoards.end(),
