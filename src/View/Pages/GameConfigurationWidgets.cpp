...
GameConfigurationWidgets::GameConfigurationWidgets(
    ...
    : ctx(_ctx)
    , allGameContexts(allCtxBuilders)
    , localPlayerCount(_localPlayerCount)
    ...
    , button_CustomConfig("Create custom configuration")
{
    for (auto&& [playerCount, gameCtx] : allGameContexts) {
        comboBox_selectablePlayerCount.append(
            std::to_string(playerCount - localPlayerCount));
    }

    comboBox_selectablePlayerCount.signal_changed().connect(sigc::mem_fun(
        *this, &GameConfigurationWidgets::new_player_count_selected));

    comboBox_selectablePlayerCount.set_active_text("2");
    comboBox_definedGameContexts.set_active_text("default");

    button_CustomConfig.signal_clicked().connect(sigc::mem_fun(
        *this, &GameConfigurationWidgets::custom_config_button_clicked));
...
void GameConfigurationWidgets::load_new_contexts()
{
    for (auto&& [playerCount, ctxBuilder] : allGameContexts) {
        ...
        for (const auto& child :
             comboBox_selectablePlayerCount.get_model()->children()) {
...
void GameConfigurationWidgets::new_player_count_selected()
{
    size_t selectedPlayerCount =
        std::stoul(comboBox_selectablePlayerCount.get_active_text());
    comboBox_definedGameContexts.remove_all();

    for (auto&& gameBuilder :
         allGameContexts[selectedPlayerCount + localPlayerCount]) {
...
cached_game_ctx_builder_t GameConfigurationWidgets::get_context_builder()
{
    ...
    auto& gameCtxBySelectedPlayerCount =
        GameCache::get().get_defined_game_contexts()[playerCount];
    auto gameCtxIt = std::find_if(
        gameCtxBySelectedPlayerCount.begin(),
        gameCtxBySelectedPlayerCount.end(), [&](const auto& gameBuilderCtx) {
            return gameBuilderCtx->get_name() == gameCtxName;
        });
