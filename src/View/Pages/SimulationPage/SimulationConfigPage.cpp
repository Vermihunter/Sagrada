...
void SimulationConfigPage::start_simulation_with_config(game_config_t gameConfig)
{
	auto gameCtx = gameConfig->gameCtxBuilder->build(
	    gameConfig->players.size(), RandomGenerator::initialize_with_seed(gameConfig->randomSeed));
	auto game =
	...
	auto controller =
	    std::make_unique<OnlyAIPlayerController>(*game, tmpConfigs, gameConfig->gameDeterministic);
	gamePlayingPage = std::make_unique<SimulationGamePlayingPage>(
	    ctx.mainWindow.homePage, true, ctx, std::move(controller), std::move(game));
...
SimulationConfigPage::with_concrete_simulation(const ViewContext& _ctx, game_config_t gameConfig)
{
