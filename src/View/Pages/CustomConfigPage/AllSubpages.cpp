...
AllSubpages::AllSubpages(const ViewContext& subPageCtx)
    : boardConfigSubpage(subPageCtx)
    , diceConfigSubpage(subPageCtx)
    , generalConfigSubpage(subPageCtx)
    , puocConfigSubpage(subPageCtx)
    , scoreConfigSubpage(subPageCtx)
    , tcConfigSubpage(subPageCtx)
...
std::unique_ptr<AllSubpageInformation> AllSubpages::collect_information()
{
    ...
    collectedInformation->generalInfo =
        generalConfigSubpage.collect_information();
    collectedInformation->boardConfig =
        boardConfigSubpage.collect_information();
    collectedInformation->diceConfig =
        diceConfigSubpage.get_entered_dice_config();
    collectedInformation->scoreCtx =
        scoreConfigSubpage.get_entered_score_config();
