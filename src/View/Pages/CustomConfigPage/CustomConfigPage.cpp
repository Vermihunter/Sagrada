...
CustomConfigPage::CustomConfigPage(const ViewContext& ctx)
    : Page(ctx)
    , tryLabel("first page")
    , hbox_SaveAndQuitButtons(true)
    , button_Save("Save")
    , button_Quit("Quit")
{
    ViewContext subpageCtx {
        SizeInPixels {ctx.size.width,
                      SizeInPixels::pixel_size_t(0.9 * ctx.size.height)},
        ctx.upperLeft, ctx.mainWindow}; // * 0.9
    allSubpages = std::make_unique<AllSubpages>(subpageCtx);

    notebook_SettingsByParts.insert_page(allSubpages->generalConfigSubpage,
                                         "General config", 0);
    notebook_SettingsByParts.insert_page(allSubpages->boardConfigSubpage,
                                         "Board config", 1);
    notebook_SettingsByParts.insert_page(allSubpages->diceConfigSubpage,
                                         "Dice config", 3);
    notebook_SettingsByParts.insert_page(allSubpages->scoreConfigSubpage,
                                         "Score config", 4);
    notebook_SettingsByParts.insert_page(
        allSubpages->playerCountRelatedInfoSubpage,
    ...
    notebook_SettingsByParts.insert_page(allSubpages->tcConfigSubpage,
                                         "Tool card config", 7);
    ...
    fixed.put(hbox_SaveAndQuitButtons, (ctx.size.width - 150) / 2,
              ctx.size.height - 40 - 5);
...
void CustomConfigPage::save_button_clicked()
{
    ...
    if (collectedInformation->playerCountRelatedInfo.empty()) {
        return ctx.mainWindow.display_error_message(
            "Missing information!", "No player configuration provided!");
    }

    if (collectedInformation->generalInfo->contextName.empty()) {
        return ctx.mainWindow.display_error_message(
            "Missing information!", "Enter a name for the game context!");
    }
    ...
    try {
        gameCtxBuilder = GameContextBuilder::create(
            ...
            std::move(collectedInformation->puocCtx),
            std::move(collectedInformation->tcCtx),
            collectedInformation->boardConfig->boardConfig);

        gameCtxBuilder
            ->add_dice_config(std::move(collectedInformation->diceConfig))
            .add_wpc_choice_per_player(
                collectedInformation->generalInfo->wpcPerPlayer)
            .add_number_of_rounds(
                collectedInformation->generalInfo->numberOfRounds)
            .set_name(collectedInformation->generalInfo->contextName)
            .add_score_ctx(std::move(collectedInformation->scoreCtx))
            .add_selectable_wpc(
                collectedInformation->boardConfig->selectableWpc);
    }
    ...
    for (auto&& playerRelatedInfo :
         collectedInformation->playerCountRelatedInfo) {
        auto playerCountCtxIt =
            definedCtxBuilders.find(playerRelatedInfo.playerCount);
        if (playerCountCtxIt == definedCtxBuilders.end()) {
            definedCtxBuilders.insert(
                {playerRelatedInfo.playerCount,
    ...
    std::ofstream file(gameConfigDir + "/" +
                       collectedInformation->generalInfo->contextName +
                       ".json");
