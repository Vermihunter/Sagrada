...
BoardConfigSubpage::BoardConfigSubpage(const ViewContext& ctx)
    ...
    , label_Dimensions("Choose board dimensions!")
    , vbox_Width(true)
    , label_Width("Width")
    , vbox_Height(true)
    , label_Height("Height")
    , hbox_Dimensions(true)
    , label_BoardTypeChooser("Choose a board type!")
    , vbox_BoardTypeChooser(true)
    , button_ConfigureBoardOptions("Select boards by current config")
    , vbox_Config(true)
    , hbox_Pager(true)
    , button_Next("Next")
    , button_Prev("Prev")
    ...
    , currBoardConfig(nullptr)
{
    ...
    /** Board type */
    for (auto&& [boardTypeName, boardTypePtr] :
         BoardType::get_all_board_types()) {
    ...
    /** Pager settings */
    button_Prev.signal_clicked().connect(sigc::bind(
        sigc::mem_fun(*this, &BoardConfigSubpage::change_page_number), -1));

    button_Next.signal_clicked().connect(sigc::bind(
        sigc::mem_fun(*this, &BoardConfigSubpage::change_page_number), 1));
...
void BoardConfigSubpage::change_page_number(int changeVal)
{
    ...
    button_Next.set_sensitive(
        (size_t(currPageNumber + 1) <
         (selectableWPCBySelectedConfig.size() / wpcChoicePerPage)));
...
BoardConfig* BoardConfigSubpage::get_board_config()
{
    ...
    BoardType* boardType =
        BoardType::get_type(cb_BoardTypeChooser.get_active_text());
...
void BoardConfigSubpage::create_wpc(const wpc_c& creatableWPC)
{
    ...
    size_t leftAndRightAlign =
        (ctx.size.width - (wpcChoicePerPage * (wpcSize.width + 10))) / 2;
    size_t ind = 0;
    for (auto&& wpc : creatableWPC) {
        selectableWPCBySelectedConfig.push_back(SelectableWPCConfig {240, wpc});

        auto& lastlyAddedTestView =
            selectableWPCBySelectedConfig[selectableWPCBySelectedConfig.size() -
                                          1];
...
board_config_subpage_info_t BoardConfigSubpage::collect_information()
{
    auto selectedFilter = std::views::filter(
        [](const SelectableWPCConfig& obj) { return obj.is_selected(); });
    auto wpcConfigToCtx = std::views::transform(
        [](const SelectableWPCConfig& obj) { return obj.wpc; });
    ...
    board_config_subpage_info_t enteredData =
        std::make_unique<BoardConfigSubpageInformation>();

    std::ranges::copy(selectableWPCBySelectedConfig | selectedFilter |
                          wpcConfigToCtx,
