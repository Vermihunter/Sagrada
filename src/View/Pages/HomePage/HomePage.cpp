...
HomePage::HomePage(
    const ViewContext& ctx /*, game_mode_t gameMode, tournament_config_t tournamentConfig*/)
    : Page(ctx), button_LocalGame("Start game"), button_Simulation("Start simulation"),
      button_Tournament("Start tournament")
{
	...
	button_LocalGame.signal_clicked().connect(sigc::mem_fun(*this, &HomePage::start_local_clicked));
	...
	homePageLocalFixed.put(starterButtonsBox, (ctx.size.width - 150) / 2, ctx.size.height / 2);
...
void HomePage::start_local_clicked()
{
	if (!localPlayerSubpage) {
		localPlayerSubpage = std::make_unique<LocalPlayerGameTypeChoosingSubpage>(ctx);
