...
LocalPlayerGameTypeChoosingSubpage::LocalPlayerGameTypeChoosingSubpage(const ViewContext& ctx)
    : Page(ctx), button_LocalPlayerVsAi("Computer"), button_Back("back")
{
	button_LocalPlayerVsAi.signal_clicked().connect(
	    sigc::mem_fun(*this, &LocalPlayerGameTypeChoosingSubpage::computer_button_clicked));
	...
	fixed.put(vbox_GameTypeOptions, (ctx.size.width - 200) / 2, ctx.size.height / 2);
	add(fixed);
	show_all_children();
}
...
void LocalPlayerGameTypeChoosingSubpage::computer_button_clicked()
{
	if (!configChoosingPage) {
		configChoosingPage = std::make_unique<LocalPlayerGameConfigurationSubpage>(ctx);
