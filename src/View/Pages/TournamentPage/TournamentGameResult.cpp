...
TournamentGameResult::TournamentGameResult(
    TournamentPage& _parentPage, const ViewContext& _ctx,
    const ViewContext& _simulationPageCtx,
    const TournamentGameInformation& _tournamentInfo, size_t _gameCount)
    : parentPage(_parentPage)
    , ctx(_ctx)
    , simulationPageCtx(_simulationPageCtx)
    , tournamentInfo(_tournamentInfo)
    , label_GameCount(std::to_string(_gameCount))
    ...
    , button_SimulateGame("Simulate game")
{
    ...
    auto& winnerAIPlayer =
        tournamentInfo.controller->get_ai_players().at(winnerPlayer->ID);
    label_Winner.set_text(
        tournamentInfo.controller->get_player_to_config_mapper()
...
void TournamentGameResult::simulate_button_clicked()
{
    auto gameCtx = tournamentInfo.gameInfo->gameCtxBuilder->build(
        ...
        RandomGenerator::initialize_with_seed(
            tournamentInfo.gameInfo->randomSeed));
    auto game = std::make_unique<Game>(
        std::move(gameCtx), DefaultGameCreator::get_default_game_state());

    auto controller = std::make_unique<OnlyAIPlayerController>(
        *game, tournamentInfo.aiConfigs,
        tournamentInfo.gameInfo->gameDeterministic);

    gameSimulatingPage = std::make_unique<SimulationGamePlayingPage>(
        parentPage, false, simulationPageCtx, std::move(controller),
        std::move(game));
