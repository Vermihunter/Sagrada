...
TournamentResultPage::TournamentResultPage(TournamentPage& _parentPage,
                                           const ViewContext& _ctx,
                                           size_t _resultsPerPage)
    : parentPage(_parentPage)
    , ctx(_ctx)
    , resultsPerPage(_resultsPerPage)
    , hbox_ColumnNames(true)
    , label_GameCountTitle("Game count")
    , label_UsedSeedTitle("Used seed")
    , label_GameWinnerTitle("Game winner")
    , label_SimulateButtonTitle("Start simulation")
{
...
void TournamentResultPage::add_game_result(
    const TournamentGameInformation& gameInfo, size_t gameCount)
{
    gameResults.push_back(std::make_unique<TournamentGameResult>(
        parentPage, ctx, ctx.with_reduced_height(1 / 0.9), gameInfo,
        gameCount));
    gameResults.back()->set_size_request(
        ctx.size.width - 10, (ctx.size.height - 55 - 70) / resultsPerPage);
    vbox_GameResultsTable.pack_start(*gameResults.back(),
