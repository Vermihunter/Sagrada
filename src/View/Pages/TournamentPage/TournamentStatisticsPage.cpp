...
TournamentStatisticsPage::TournamentStatisticsPage(
    const ViewContext& _ctx, TournamentStatistics& _tournamentStat)
    : ctx(_ctx)
    , tournamentStat(_tournamentStat)
    ...
    , label_PlayerStatisticsTitle("Player statistics")
    , label_PlayerName("AI player type")
    , label_WinCount("Win count")
    , label_ReceivedPlacementPoints("Placement points")
    , label_AveragePoints("Average points")
{
    ...
    WilsonScoreInterval wilsonScoreInterval =
        tournamentStat.playerStatistics.at(tournamentWinner)
            .get_wilson_score_interval_with_98_confidence();

    std::ostringstream wilsonScoreTextOss;
    wilsonScoreTextOss << tournamentWinner->get_name() << " wins "
                       << std::setprecision(3)
                       << wilsonScoreInterval.interval_low * 100 << "% - "
                       << wilsonScoreInterval.interval_high * 100
                       << "% games with " << wilsonScoreInterval.confidence
                       << "% confidence";
    label_WilsonScoreIntervalForWinningPlayer.set_text(
        wilsonScoreTextOss.str());
    label_WilsonScoreIntervalForWinningPlayer.get_style_context()->add_provider(
        pageCss, GTK_STYLE_PROVIDER_PRIORITY_APPLICATION);
    label_WilsonScoreIntervalForWinningPlayer.set_name(
        "tournament_header_label");
...
void TournamentStatisticsPage::add_player_statistics(
    const AI_PlayerConfig* aiConfig, const PlayerStatistics& playerStat,
    size_t playerInd)
{
    playerStatisticsWidgets.push_back(
        PlayerStatisticsWidgets {aiConfig, playerStat, pageCss});

    PlayerStatisticsWidgets& playerStatWidgets = playerStatisticsWidgets.back();

    grid_PlayerStatistics.attach(playerStatWidgets.label_PlayerName, 0,
                                 playerInd + 2);
    grid_PlayerStatistics.attach(playerStatWidgets.label_WinCount, 1,
                                 playerInd + 2);
    grid_PlayerStatistics.attach(
        playerStatWidgets.label_ReceivedPlacementPoints, 2, playerInd + 2);
    grid_PlayerStatistics.attach(playerStatWidgets.label_AveragePoints, 3,
                                 playerInd + 2);
}
...
void TournamentStatisticsPage::initialize_table()
{
    ...
    initialize_column_with_css(grid_PlayerStatistics,
                               label_PlayerStatisticsTitle,
                               overallTableTitleRow, 0, 4, true);
    initialize_column_with_css(grid_PlayerStatistics, label_PlayerName,
                               columnTitleLabelRow, 0, 1, true);
    initialize_column_with_css(grid_PlayerStatistics, label_WinCount,
                               columnTitleLabelRow, 1, 1, true);
    initialize_column_with_css(grid_PlayerStatistics,
                               label_ReceivedPlacementPoints,
                               columnTitleLabelRow, 2, 1, true);
    initialize_column_with_css(grid_PlayerStatistics, label_AveragePoints,
                               columnTitleLabelRow, 3, 1, true);
    ...
    initialize_column_with_css(grid_OverallStatistics,
                               label_OverallStatisticsTitle,
                               overallTableTitleRow, 0, 2, true);
    initialize_column_with_css(grid_OverallStatistics,
                               label_TotalGameCountTitle, 1, 0, 1, true);
    initialize_column_with_css(grid_OverallStatistics, label_TotalGameCount, 1,
                               1, 1, false);
}

void TournamentStatisticsPage::initialize_column_with_css(
    Gtk::Grid& grid, Gtk::Label& label_ColumnTitle, size_t index_Row,
    size_t index_Column, size_t width_Column, bool isHeader)
{
    grid.attach(label_ColumnTitle, index_Column, index_Row, width_Column);
    if (isHeader) {
        label_ColumnTitle.set_name("tournament_header_label");
    }
    label_ColumnTitle.get_style_context()->add_provider(
        pageCss, GTK_STYLE_PROVIDER_PRIORITY_APPLICATION);
}