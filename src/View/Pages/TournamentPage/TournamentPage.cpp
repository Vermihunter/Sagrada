...
TournamentPage::TournamentPage(const ViewContext& _ctx, tournament_config_t _tournamentConfig)
    : Page(_ctx), remainingGames(_tournamentConfig->gameCount),
      currSeed(_tournamentConfig->gameConfig->randomSeed),
      statistics(_tournamentConfig->gameConfig->players),
      tournament(std::move(_tournamentConfig), statistics), tournamentStopped(false),
      results(*this, ctx.with_reduced_height(0.90), 10), alignment_StatisticsPageMiddle(0.5, 0.0),
      button_SwitchToResults("Show results"),
      button_SwitchToOverallStatistics("Show overall statistics"), button_Quit("Quit")
{
	std::string title = "Tournament between ";
	title += tournament.get_config()->gameConfig->players[0]->get_name() + " player";
	...
	alignment_MiddleTitle.set_size_request(ctx.size.width, ctx.size.height * 0.10);
	...
	button_SwitchToOverallStatistics.signal_clicked().connect(
	    sigc::mem_fun(*this, &TournamentPage::show_overall_statistics_button_clicked));

	button_SwitchToOverallStatistics.set_sensitive(false);
	button_SwitchToOverallStatistics.set_size_request(200, 50);

	button_SwitchToResults.signal_clicked().connect(
	    sigc::mem_fun(*this, &TournamentPage::show_game_results_button_clicked));
	...
	fixed.put(hbox_DownButtons, (ctx.size.width - 400) / 2, ctx.size.height - 50 - 5);
	add(fixed);
	show_all_children();

	constexpr unsigned int timeOutInMs = 150;
	firstTimeoutHandler                = Glib::signal_timeout().connect(
	                   sigc::mem_fun(*this, &TournamentPage::on_first_starting_timeout), timeOutInMs);
...
bool TournamentPage::on_game_runner_timeout()
{
	...
	playedGames.push_back(tournament.launch_game(currSeed++, gameCount, aiConfigs));
	results.add_game_result(*playedGames.back(), gameCount);

	bool allGamesRun = (--remainingGames == 0);
	if (allGamesRun) {
		statisticsPage = std::make_unique<TournamentStatisticsPage>(ctx, statistics);
...
bool TournamentPage::on_first_starting_timeout()
{
	...
	gameRunnerTimeoutHandler           = Glib::signal_timeout().connect(
	              sigc::mem_fun(*this, &TournamentPage::on_game_runner_timeout), timeOutInMs);
