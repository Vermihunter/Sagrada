...
WPCChoosingPage::WPCChoosingPage(const ViewContext& ctx, std::unique_ptr<Game> _game,
                                 const ai_player_config_c& ai_configs, size_t localPlayerCount)
    : Page(ctx), label_Title("Choose a Window pattern card!"), alignment_Title(0.5, 0.0),
      button_Quit("Quit"), button_Start("Start"), game(std::move(_game)), wpcSize(330, 300),
      selectedWPC(nullptr), currLocalPlayerInd(0)
{
	controller = LocalPlayerController::with_players(localPlayerCount, ai_configs, *game);
	...
	commonPageElements = CommonGameElements::create(controller->get_game(),
	                                                controller->get_local_player_ids(), cardWidth);

	vbox_allCards.pack_start(commonPageElements->tcViews);
	vbox_allCards.pack_start(commonPageElements->puocViews, Gtk::PACK_EXPAND_PADDING, 10);
	...
	fixed.put(hbox_Buttons, (ctx.size.width - 200) / 2, ctx.size.height - 50 - 10);
...
void WPCChoosingPage::quit_button_clicked()
{
...
void WPCChoosingPage::start_button_clicked()
{
	if (!selectedWPC) {
		...
		dialog.set_secondary_text("You have to select a Window pattern card by clicking on it!");
		dialog.run();
		return;
	}

	controller->handle_local_player_WPC_choice(
	    controller->get_local_player_ids()[currLocalPlayerInd], selectedWPC->get_ctx());
	...
	gamePlayingPage = std::make_unique<LocalGamePlayingPage>(
	    ctx, std::move(controller), std::move(commonPageElements), std::move(game));
...
{
	auto& playerInfo = GameInfo::get_player_with_id(controller->get_game(), localPlayerID);
	...
	commonPageElements->procViews.pack_start(*commonPageElements->localPlayerProcCards.back());
}

void WPCChoosingPage::add_wpc_for_player(ID_t localPlayerID)
{
	auto&& [wpcBegin, wpcEnd] = controller->get_wpc_choices_for_local_player_with_id(localPlayerID);
	size_t wpcCount           = std::distance(wpcBegin, wpcEnd);
	localPlayerWPCChoices.push_back(Gtk::Table{wpcPerRow, guint(wpcCount / wpcPerRow)});
	Gtk::Table& lastTable = localPlayerWPCChoices.back();

	size_t ind = 0;
	for (auto it = wpcBegin; it != wpcEnd; ++it) {
		wpcChoices.push_back(std::make_unique<RectangularBoardView>(**it, wpcSize));
	...
	lastTable.hide();
	fixed.put(lastTable, leftMargin, (ctx.size.height - 2 * wpcSize.height) / 2);
}