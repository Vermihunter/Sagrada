...
RectangularBoardView::RectangularBoardView(const BoardContext& _boardCtx,
...
void RectangularBoardView::connect_to_player(
    const PlayerInformation* _playerInfo)
{
    if (!playerInfo) {
        size.height += 20;
...
void RectangularBoardView::select_fields(const board_ind_c& indices,
                                         Color* color)
{
...
void RectangularBoardView::handle_attribute_placing_click(GdkEventButton* event)
{
    ...
    const size_t onePieceWidth =
        rectangleSize.width + distanceBetweenRectanglesInPixels;
    const size_t onePieceHeight =
        rectangleSize.height + distanceBetweenRectanglesInPixels;
    ...
    if (colInd != size_t((event->x + distanceBetweenRectanglesInPixels) /
                         (onePieceHeight)) ||
    ...
    if (rowInd != size_t((event->y + distanceBetweenRectanglesInPixels) /
                         (onePieceHeight)) ||
...
{
    rectangleSize.width =
        (size.width - 2 * distanceXFromBorderInPixels -
    ...
    for (auto&& boardInd : boardCtx.config->type->get_all_indices()) {
        fields[boardInd.row][boardInd.col] = std::make_unique<BoardFieldView>(
            Point {distanceXFromBorderInPixels +
                       boardInd.col * rectangleSize.width +
                       boardInd.col * distanceBetweenRectanglesInPixels,
                   distanceYFromBorderInPixels +
                       boardInd.row * rectangleSize.height +
                       boardInd.row * distanceBetweenRectanglesInPixels +
                       additionalHeight},
...
void RectangularBoardView::draw_base(const Cairo::RefPtr<Cairo::Context>& cr)
{
    ...
    if (isSelected) {
        ...
        cr->arc(x + width - radius, y + radius, radius, -90 * degrees,
                0 * degrees);
        cr->arc(x + width - radius, y + height - radius, radius, 0 * degrees,
                90 * degrees);
        cr->arc(x + radius, y + height - radius, radius, 90 * degrees,
                180 * degrees);
    ...
    cr->move_to(0, 0);
    cr->begin_new_sub_path();
    cr->arc(x + width - radius, y + radius + (isSelected ? x / 2 : 0), radius,
            -90 * degrees, 0 * degrees);
    cr->arc(x + width - radius, y + height - radius, radius, 0 * degrees,
            90 * degrees);
    cr->arc(x + radius + (isSelected ? x / 2 : 0), y + height - radius, radius,
            90 * degrees, 180 * degrees);
    cr->arc(x + radius + (isSelected ? x / 2 : 0),
            y + radius + (isSelected ? x / 2 : 0), radius, 180 * degrees,
            270 * degrees);
    cr->close_path();
...
void RectangularBoardView::draw_title(const Cairo::RefPtr<Cairo::Context>& cr)
{
    ...
    /** Center align */
    cr->move_to(
        (size.width) / 2 - te.x_bearing - te.width / 2,
...
void RectangularBoardView::draw_hardness_circles(
    const Cairo::RefPtr<Cairo::Context>& cr)
{
    ...
    cr->set_source_rgb(1.0, 1.0, 1.0);
    cr->move_to(
        size.width - localDistanceXFromBorderInPixels,
     ...
     */
    auto hardnessCircleCount =
        (playerInfo ? playerInfo->favorTokensLeft : boardCtx.hardness);
...
void RectangularBoardView::draw_player_name(
    const Cairo::RefPtr<Cairo::Context>& cr)
{
    /** White title */
    cr->set_source_rgb(1.0, 1.0, 1.0);
