...
PlayerOrderDisplayer::PlayerOrderDisplayer(const GameState& _gameState, const player_c& _players,
                                           const SizeInPixels _size)
    : gameState(_gameState), players(_players), size(_size), defaultNameColor(black),
...
bool PlayerOrderDisplayer::on_draw(const Cairo::RefPtr<Cairo::Context>& cr)
{
	...
	for (auto&& turnInfo : gameState.currRoundInfo->turnInfos) {
		const auto&  player = GameInfo::get_player_with_id_from_players(players, turnInfo.playerId);
		std::string* moveName = (ind < (gameState.currRoundInfo->turnInfos.size() / 2))
		                            ? &firstMoveNames
		                            : &secondMoveNames;
	...
	cr->move_to((size.width / 2) - (150 / 2) - 10 - teFirstNames.x_bearing - teFirstNames.width,
	...
	for (auto&& turnInfo : gameState.currRoundInfo->turnInfos) {
		...
		if (!firstName) {
			cr->set_source_rgb(defaultNameColor.r, defaultNameColor.g, defaultNameColor.b);
			cr->show_text(" - ");
		}
		const Color& nameColor =
		    (ind == gameState.currPlayerIndex) ? playerOnMoveNameColor : defaultNameColor;
