...
local_player_controller_t LocalPlayerController::with_players(
    size_t localPlayerCount, const ai_player_config_c& configs, Game& game,
    const bool gameDeterministic)
{
    auto controller =
        std::unique_ptr<LocalPlayerController>(new LocalPlayerController(game));

    auto& gameCtx = game.get_ctx();
    auto concreteAiPlayerNameIndices =
        controller->game.get_ctx()->rnd->generate_random_id_order(
    ...
    for (auto&& player : gameCtx->players) {
        if (playerInd < localPlayerCount) {
            ...
            controller->AI_PlayersByID[player->ID] =
                AI_Player::initialize_player(
                    controller->get_game(),
                    *configs[playerInd - localPlayerCount], player,
                    gameDeterministic);
            player->name =
                AI_PlayerNames[concreteAiPlayerNameIndices[playerInd -
                                                           localPlayerCount]];
        }
...
LocalPlayerController::get_wpc_choices_for_local_player_with_id(
    ID_t localPlayerID)
{
...
void LocalPlayerController::handle_local_player_WPC_choice(
    ID_t localPlayerId, const BoardContext& boardCtx)
{
...
{
    return std::find(localPlayerIDs.begin(), localPlayerIDs.end(), id) !=
           localPlayerIDs.end();
}
