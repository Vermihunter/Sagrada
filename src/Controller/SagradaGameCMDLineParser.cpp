...
game_config_t SagradaGameCMDLineParser::get_game_config()
{
    ...
    auto ctxName = get<std::string>(
        "-mode"); // (parent ? parent->get<std::string>("-mode") : );
    clonedGameConfig->gameCtxBuilder =
        GameCache::get().get_context(playerCount, ctxName);
    if (!clonedGameConfig->gameCtxBuilder) {
        throw std::runtime_error("There is no context defined for " +
