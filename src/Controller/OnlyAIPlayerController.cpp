    ...
    : ControllerWithAIPlayer(_game)
{
    ...
    for (auto&& player : game.get_ctx()->players) {
        player->name = "Player" + std::to_string(playerInd) + "(" +
                       configs[playerInd]->get_monogram() + ")";
        auto ai_player = AI_Player::initialize_player(
            game, *configs[playerInd], player, gameDeterministic);
...
only_ai_controller_t OnlyAIPlayerController::run_whole_game(
    Game& _game, const ai_player_config_pc& configs)
{
