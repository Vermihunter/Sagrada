...
bool BoardTextFileReader::add_hardness(const std::string& hardnessStr)
{
	...
	if (!uintConversionSuccessful) return false;
...
bool to_uint(const std::string& str, unsigned int& dest)
{
	try {
		...
		if (pos != str.size()) {
			throw std::invalid_argument{"\"" + str + "\" is not a correct unsigned intiger!"};
		}
	}
...
{
	const std::string errorMsg = "Invalid Config on line " + std::to_string(linesRead) +
	...
	if (!to_uint(configParts[0], boardWidth) || !to_uint(configParts[1], boardHeight)) {
		LOG_E(errorMsg);
		return false;
	}
	...
	currBoardInputHandler = type->construct_input_handler(currCtx->data, currCtx->config);
...
{
	BoardRowParsingFeedBack feedBack = currBoardInputHandler->handle_line(dataRow);
...
bool BoardTextFileReader::process_line_divider(const std::string& line, wpc_cache_t& dest)
{
	if (line != cardDivider) {
