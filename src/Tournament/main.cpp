...
std::string generate_output_name_for_config(const tournament_config_t& config)
{
	...
	oss << "tournament_results/Date_" << Date::get_curr_date() << "_Time_" << Date::get_curr_time()
	    << "_"
	    << "G_" << config->gameCount << "_S_" << config->gameConfig->randomSeed << "_P_"
	    << config->gameConfig->players[0]->get_name();
...
tournament_config_t parse_config(int argc, char** argv)
{
	...
	SagradaGameCMDLineParser tournamentParser(tConfig->logStream, true, nullptr, "build/tournament",
	...
	tournamentParser.add_epilog("Examples:\n"
	                            "\n"
	...
	if (!csvoutput && tConfig->saveMoveStatistics) {
		exit_with_err_message("To save move statistics, the -csv option has to be defined",
		                      tournamentParser);
	}

	tConfig->gameCount = tournamentParser.get<unsigned int>("-n");
	if (tConfig->gameCount == 0) {
		exit_with_err_message("Game count has to be a positive integer!", tournamentParser);
	}
	...
	if (csvoutput) {
		...
		for (size_t i = 0; i < tConfig->gameConfig->players.size(); ++i) {
			...
			csvGameResultsHeader.push_back("Player " + playerCountStr + " score");
		}
		...
		tConfig->tournamentPrinters.push_back(std::make_unique<TournamentCSVPrinter>(
		    *tConfig,
		    TournamentCSVPrinterConfig::create_default(tConfig->output, csvGameResultsHeader)));

		const std::string csvOutputDir =
		    (*csvoutput == "default") ? generate_output_name_for_config(tConfig) : *csvoutput;
		tConfig->tournamentPrinters.push_back(std::make_unique<TournamentCSVPrinter>(
		    *tConfig,
		    TournamentCSVPrinterConfig::create_default(csvOutputDir, csvGameResultsHeader)));
	}
	return tConfig;
}
