...
std::vector<std::vector<size_t>> get_all_permutations(size_t N)
{
	...
	do result.push_back(numbers);
	while (std::next_permutation(numbers.begin(), numbers.end()));

	return result;
}

Tournament::Tournament(tournament_config_t _config, TournamentStatistics& _destStatistics)
...
TournamentStatistics Tournament::run(tournament_config_t config)
{
	...
	if (config->allDirections) {
		...
		for (auto&& permutation : allAIPermutations) {
			...
			for (auto&& permutationInd : permutation) {
				lastPermutation.push_back(config->gameConfig->players[permutationInd].get());
			}
		}
	}
	else {
	...
	for (size_t i = 0; i < size_t(model.config->gameCount) / aiConfigs.size(); ++i) {
                                               ...
                                               ai_player_config_pc& aiConfigs)
{
	auto gameCtx = config->gameConfig->gameCtxBuilder->build(
	    config->gameConfig->players.size(), RandomGenerator::initialize_with_seed(seed));

	tournament_game_info_t gameInformation = TournamentGameInformation::create(aiConfigs);
	...
	gameInformation->gameInfo->gameCtxBuilder = config->gameConfig->gameCtxBuilder;

	gameInformation->controller = std::make_unique<OnlyAIPlayerController>(
	    *gameInformation->game, aiConfigs, config->gameConfig->gameDeterministic);
	auto& game           = gameInformation->controller->get_game();
	auto& AI_players     = gameInformation->controller->get_ai_players();
	auto& playerToConfig = gameInformation->controller->get_player_to_config_mapper();

	while (!game.get_curr_state()->hasEnded) {
		ID_t playerOnMoveId = game.get_curr_state()->player_on_move_id();
		auto playerConfig   = playerToConfig.at(AI_players.at(playerOnMoveId).get());
		...
		if (config->saveMoveStatistics) {
			for (auto&& tournamentPrinter : config->tournamentPrinters) {
				tournamentPrinter->print_move_statistics(
				    gameInformation->controller->get_next_ai_player()->get_possible_moves(game),
...
std::vector<tournament_game_info_t>
Tournament::launch_games(int seed, size_t gameGroupCount,
                         std::vector<ai_player_config_pc>& aiConfigs)
{
	std::vector<tournament_game_info_t> results;
	size_t                              ind = 0;
	for (auto&& aiConfig : aiConfigs) {
		results.push_back(launch_game(seed, gameGroupCount * aiConfigs.size() + ind, aiConfig));
...
game_result_t Tournament::update_statistics(tournament_game_info_t& gameInformation)
{
	...
	for (auto&& [player, playerConfig] :
		...
		auto& playerScore =
		    *std::find_if(gameInformation->finalScores.begin(), gameInformation->finalScores.end(),
		                  [&](const eval_state_t& score) {
		...
		gameResult->pointsByPlayers.push_back({playerConfig, playerScore->get_total_points()});
	}

	ai_player_t&     winnerPlayer = AI_players[gameInformation->finalScores[0]->get_player()->ID];
...
void Tournament::print_results(tournament_game_info_t& gameInfo, game_result_t& gameResult,
                               int seed)
{
