...
MinimaxPlayerConfig::MinimaxPlayerConfig(size_t _depth,
                                         const HeuristicConstants& _constants,
                                         int _worldCount)
    : depth(_depth), constants(_constants), worldCount(_worldCount)
{
    nameWithDepth =
        type + std::to_string(depth) +
        (_worldCount > 0 ? ",worlds=" + std::to_string(worldCount) : "");
}

ai_player_t MinimaxPlayerConfig::create_player(
    const AIPlayerContext& _ctx) const
{
