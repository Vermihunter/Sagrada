...
ID_t RandomPlayer::choose_window_pattern_card(wpc_it_t iBegin, wpc_it_t iEnd)
{
	...
	return ctx.game.get_ctx()->rnd->random_in_interval(0, choosingOptionCount - 1);
}
...
move_t RandomPlayer::make_next_move()
{
	...
	ID_t randomMoveIndex = ctx.game.get_ctx()->rnd->random_in_interval(0, all_moves.size() - 1);
	free_all_but(all_moves.begin(), all_moves.end(), all_moves[randomMoveIndex]);
