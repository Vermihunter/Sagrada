...
move_t MCTSPlayer::make_next_move()
{
    ...
    if (ctx.gameDeterministic) {
        ...
        return root->get_most_visited_move()->clone(
            *root->get_game()->get_ctx()->supply, *ctx.game.get_ctx()->supply);
    }

    // Otherwise, we create the worlds and run the same amount of iterations in
    // all of them
    std::vector<game_t> worlds =
        create_worlds(playerCtx->determinizingWorldCount.value());
    ...
    return std::get<0>(moves[0])->clone(
        *moveSourceNode->get_game()->get_ctx()->supply,
...
std::unique_ptr<MCTS_Node> MCTSPlayer::get_root_from_world(game_t gameWorld)
{
    /** Algorithm */
    auto root =
        std::make_unique<MCTS_Node>(std::move(gameWorld), nullptr, *playerCtx);
...
std::vector<game_t> MCTSPlayer::create_worlds(int worldCount)
{
    ...
    for (int i = 0; i < worldCount; ++i) {
        worlds.push_back(
            ctx.game.clone_with_pseudo_random_future(ctx.ID, 73 + i));
    }
...
MCTS_Node& MCTSPlayer::select(MCTS_Node& _root)
{
    ...
    while (currNode->is_fully_expanded() && !currNode->is_leaf()) {
        ...
        currNode =
            &(**std::max_element(currNodeChildren.begin(),
                                 currNodeChildren.end(), nodesByHighestUCT));
    }
