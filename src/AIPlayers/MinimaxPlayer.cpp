...
MinimaxPlayer::MinimaxPlayer(const AIPlayerContext& _ctx, size_t _depth,
                             const HeuristicConstants& _constants,
                             int _worldCount)
    : AI_Player(_ctx)
    , depth(_depth)
...
move_t MinimaxPlayer::make_next_move()
{
    if (ctx.gameDeterministic) {
        return minimax_algorithm(ctx.game, depth, INT_MIN, INT_MAX,
                                 maximizingPlayer, nullptr)
            .first;
    }
    ...
    for (int i = 0; i < worldCount; ++i) {
        game_t sampleWorld =
            ctx.game.clone_with_pseudo_random_future(ctx.ID, i + 99);

        std::vector<std::pair<move_t, int>> currWorldResults;
        minimax_algorithm(*sampleWorld, depth, INT_MIN, INT_MAX,
                          maximizingPlayer, &currWorldResults);
        ...
        size_t moveInd = 0;
        for (auto&& [sampleWorldMove, sampleWorldMoveHeuristicValue] :
             currWorldResults) {
    ...
    move_t clonedMove = bestMove->clone(*firstSampleWorld->get_ctx()->supply,
                                        *ctx.game.get_ctx()->supply);
    free_all_with_ind(overallResults.begin(), overallResults.end());
    return clonedMove;
}

heuristic_result_t MinimaxPlayer::get_heuristic_value_for_score(
...
heuristic_result_t MinimaxPlayer::heuristic(Game& g)
{
    ...
    for (auto&& score : scores) {
        heuristic_result_t playerScoreHeuristic =
            get_heuristic_value_for_score(score);
        if (score->get_player()->ID == ctx.ID) {
            heuristicValue += playerScoreHeuristic;
        }
        else {
            heuristicValue -= (playerScoreHeuristic /
                               constants.opponentHeuristicInfluencingFactor);
        }
    }
...
move_by_index_c& MinimaxPlayer::get_moves(Game& g, move_c& allMoves,
                                          ...
                                          move_by_index_c& backupMoves)
{
    ...
    move_by_index_c& movesToDiscover =
        (bestMoves.empty() ? backupMoves : bestMoves);
    auto& currPlayer =
...
std::pair<move_t, heuristic_result_t> MinimaxPlayer::minimax_algorithm(
    Game& g, size_t _depth, int alpha, int beta,
    MinimaxPlayerType<heuristic_result_t>& player,
    std::vector<std::pair<move_t, heuristic_result_t>>* movesWithHeuristicValue)
{
    if (_depth == 0 || g.get_curr_state()->hasEnded)
        return {nullptr, heuristic(g)};
    ...
    move_by_index_c& movesToDiscover =
        get_moves(g, possibleMoves, heuristicFilteredMovesByIndices,
    ...
    for (auto&& [move, moveInd] : movesToDiscover) {
        ...
        bool maximizingPlayerNextMove =
            (ctx.ID == g.get_curr_state()->player_on_move_id());
        ...
        auto [returnedMove, heuristicValue] = minimax_algorithm(
            g, _depth - 1, alpha, beta, *nextLayerPlayer, nullptr);
    ...
    if (depth != _depth) {
        ...
        move_by_index_c& unusedMoves =
            (&movesToDiscover == &backupMovesByIndices
                 ? heuristicFilteredMovesByIndices
                 : backupMovesByIndices);
