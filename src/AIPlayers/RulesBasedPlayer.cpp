...
RulesBasedPlayer::RulesBasedPlayer(const AIPlayerContext& _ctx,
                                   strategy_t _strategy)
    : AI_Player(_ctx), constants(defaultConstants), strategy(_strategy)
{
    // ctor
}

ID_t RulesBasedPlayer::choose_window_pattern_card(wpc_it_t iBegin,
                                                  wpc_it_t iEnd)
{
    auto fieldRestrictionMatchesProcColor =
        [&](const auto& field) { // std::pair<BoardIndex, board_field_t>
            return field->get_color_restriction().has_value() &&
                   field->get_color_restriction().value() ==
                       ctx.privateObjectiveCard->get_color();
        };
...
{
    if (strategy == strategy_t::ALL_MOVES ||
        ctx.game.get_curr_state()->previousToolCardMove) {
    ...
    move_c allDtfmMoves;
    ctx.game.get_ctx()->basicMoveHandler->append_basic_moves(
        allDtfmMoves, ctx.game, ctx.ID, dtfm_mode_t::DTFM_MODE_DEFAULT);
...
move_t RulesBasedPlayer::make_next_move()
{
    ...
    auto positionBetteringRelocatingMove = check_good_relocating_exists(
        ctx.game, positionBetteringToolCardMoves, scores);
    if (positionBetteringRelocatingMove) {
        free_all_but(all_moves.begin(), all_moves.end(),
                     positionBetteringRelocatingMove);
        return positionBetteringRelocatingMove;
    }

    auto [bestDtfm, dtfmHeuristic] =
        get_best_dtfm(ctx.game, dieToFieldMoves, scores);

    move_t bestMove = bestDtfm;
    if (!bestMove || dtfmHeuristic < 0) {
        if (!randomContainingToolCardMoves.empty()) {
            bestMove = randomContainingToolCardMoves
                [ctx.game.get_ctx()->rnd->random_in_interval(
...
move_t RulesBasedPlayer::check_good_relocating_exists(
    Game& g, move_c& moves, const eval_state_c& scores)
{
    ...
    for (auto&& move : moves) {
        if (move->t == Move::type::TOOL_CARD &&
            !((tool_card_move_t)(move))
                 ->consists_of_multiple_submoves()
                 .isSingleMoveToolCard) {
            continue;
        }
        ...
        auto& newMyScore = *std::find_if(
            clonedScores.begin(), clonedScores.end(), ScoreByPlayerId {ctx.ID});

        int uncompletableFieldCountDifference =
            (int)(myScore->get_overall_board_state()
                      .uncompletableFieldCount) - // Original
                                            ...
                                            const eval_state_c& evalStates)
{
    ...
    const eval_state_t& myScore = *std::find_if(
        evalStates.begin(), evalStates.end(), ScoreByPlayerId {ctx.ID});

    // the last two parts should be multiplied with the corresponding evaluation
    // constants from the score context object
    int totalPoints =
        myScore->get_total_points() - myScore->get_empty_fields_points() -
    ...
    for (auto&& move : dieToFieldMoves) {
        ...
        int uncompletableFieldCountDifference =
            (int)(myScore->get_overall_board_state()
                      .uncompletableFieldCount) - // Original
            (int)(newMyScore->get_overall_board_state()
                      .uncompletableFieldCount); // After move

        // There was a move before that produced less new uncompletable fields
        if (uncompletableFieldCountDifference <
            currMinNewUnfillableFieldCount) {
            continue;
        }

        currMinNewUnfillableFieldCount = std::max(
            currMinNewUnfillableFieldCount, uncompletableFieldCountDifference);
        ...
        if (dtfm->die->get_color() == ctx.privateObjectiveCard->get_color()) {
            newStateValue +=
                (((heuristic_value_t)(dtfm->die->get_value()) + 1) * 5);
        }

        int newTotalPoints =
            newMyScore->get_total_points() -
        ...
        if (newStateValue > bestStateValue ||
            ((newStateValue + 10) > bestStateValue &&
             (newTotalPoints > (totalPoints + 3)))) {
...
heuristic_value_t RulesBasedPlayer::evaluate_puoc_state(
    const eval_state_t& evalState)
{
    ...
    for (auto&& puocPattern : evalState->get_puoc_state()) {
        auto satisfactionValue =
            puocPattern.puoc.get_ctx()->get_satisfaction_value();
        for (auto&& subpattern : puocPattern.completableSubPatterns) {
            weightedPuocCompletable +=
                (satisfactionValue * std::pow((puocPattern.dicePerPattern -
                                               subpattern.diceToComplete) +
                                                  1,
                                              constants.puocCompletablePower));
        }

        puocUncompletablePoints +=
            (puocPattern.uncompletableSubPatternCount * satisfactionValue);
        puocPointsCompleted += puocPattern.totalEarnedPoints;
    }

    return (constants.completedPointsWeight * puocPointsCompleted) +
           (weightedPuocCompletable) -
