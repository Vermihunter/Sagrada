...
DefinedAIPlayers::DefinedAIPlayers()
{
	...
	definedPlayerConfigs.push_back(
	    std::make_unique<MinimaxPlayerConfig>(2, defaultConstants, MINIMAX_defaultWorldCount));
	definedPlayerConfigs.push_back(
	...
	definedPlayerConfigs.push_back(std::make_unique<MCTSPlayerConfig>(std::move(mcts_ctx)));
}

ai_player_config_t DefinedAIPlayers::construct_by_name(const std::string& aiName)
{
	if (aiName == "random") {
		...
		RulesBasedPlayer::strategy_t strategy = RulesBasedPlayer::DEFAULT_STRATEGY;
		if (match[1].length() > 0) {
	...
	// Minimax format: minimax-depth=3
		...
		std::regex pattern("^minimax-depth=(\\d+)(,worlds=(\\d+))?(?:,config_file=([^,]+))?$");
		...
		return std::make_unique<MinimaxPlayerConfig>(depth, heuristicConstants, worldCount);
	}
