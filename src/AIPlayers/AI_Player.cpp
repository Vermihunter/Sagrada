...
ai_player_t AI_Player::initialize_player(Game& game, const AI_PlayerConfig& config,
                                         const player_t& playerInfo, const bool _gameDeterministic)
{
	...
	auto [wpc_begin, wpc_end] = game.get_player_window_pattern_card_options(playerInfo->ID);
	ID_t chosenWPCIndex       = newAIPlayer->choose_window_pattern_card(wpc_begin, wpc_end);
