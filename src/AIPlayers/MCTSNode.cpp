...
MCTS_Node::MCTS_Node(game_t _game, MCTS_Node* _parent, const MCTSPlayerContext& _playerCtx)
    : game(std::move(_game)), playerCtx(_playerCtx), parent(_parent), visitCount(0), winCount(0)
{
...
uct_t MCTS_Node::get_uct()
{
	return (winCount / (visitCount * 5)) +
	       playerCtx.explorationConstant * std::sqrt(std::log(parent->visitCount) / visitCount);
}
...
MCTS_Node& MCTS_Node::expand()
{
	...
	move_t  selectedMove =
	    moveToExpand->clone(*game->get_ctx()->supply, *clonedGame->get_ctx()->supply);
	clonedGame->move_request(selectedMove);

	children.push_back(std::make_unique<MCTS_Node>(std::move(clonedGame), this, playerCtx));
	return *children.back();
}

void MCTS_Node::update(eval_state_c& scores)
{
	if (parent && scores[0]->get_player()->ID == (*parent->possibleMoves)[0].first->playerId) {
...
game_t MCTS_Node::simulate_game()
{
	...
	for (size_t i = 0; i < fasterGame.get_ctx()->players.size(); ++i) {
		AIPlayerContext aiCtx{.ID                   = i,
		                      .game                 = fasterGame,
		                      .privateObjectiveCard = fasterGame.get_ctx()->players[i]->PrOC,
	...
	while (!fasterGame.get_curr_state()->hasEnded) {
		move_t chosenMove =
		    aiPlayers[fasterGame.get_curr_state()->player_on_move_id()]->make_next_move();
...
{
	auto mostVisitedNodeCmp = [](const mcts_node_t& lhs, const mcts_node_t& rhs) {
	...
	auto mostVisitedChild =
	    std::max_element(children.cbegin(), children.cend(), mostVisitedNodeCmp);
	int mostVisitedMoveIndex = std::distance(children.cbegin(), mostVisitedChild);
...
void MCTS_Node::add_filtered_moves()
{
	...
	std::unique_ptr<move_by_index_c> backupMoves = std::make_unique<move_by_index_c>();
