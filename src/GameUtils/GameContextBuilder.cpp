...
game_context_builder_t GameContextBuilder::create(
    ...
    puoc_context_t _puocContext, tc_context_t _tcContext, board_config_t boardConfig)
{
	...
	if (_tcContext->cards_per_game() > _tcContext->get_current_game_tcs().size()) {
		throw GameContextBuildingException{
		    "There are less different card configurations than the required "
	...
	if (playerCountRelatedConfigs.empty()) {
		throw GameContextBuildingException{"No player count related config provided!"};
	}

	GameContextBuilder* tmpRawBuilder = new GameContextBuilder(
	    playerCountRelatedConfigs, std::move(_puocContext), std::move(_tcContext), boardConfig);
	return std::unique_ptr<GameContextBuilder>(tmpRawBuilder);
}

GameContextBuilder::GameContextBuilder(
    const std::vector<PlayerCountRelatedGameInformation>& _playerCountRelatedConfigs,
    puoc_context_t _puocContext, tc_context_t _tcContext, board_config_t _boardConfig)
    : playerCountRelatedConfigs(_playerCountRelatedConfigs), puocContext(std::move(_puocContext)),
      tcContext(std::move(_tcContext)), boardConfig(_boardConfig)
...
GameContextBuilder& GameContextBuilder::add_dice_config(dice_config_c _diceConfig)
{
	diceConfig = std::move(_diceConfig);
	return *this;
}

GameContextBuilder& GameContextBuilder::add_number_of_rounds(size_t _numberOfRounds)
{
	numberOfRounds = _numberOfRounds;
	return *this;
}

GameContextBuilder& GameContextBuilder::add_wpc_choice_per_player(size_t _wpcChoicePerPlayer)
{
	wpcChoicePerPlayer = _wpcChoicePerPlayer;
	return *this;
}

GameContextBuilder& GameContextBuilder::add_player(player_t player, game_context_t& gameCtx)
{
	...
	bool confictingPlayerAlreadyExists = std::any_of(
	    gameCtx->players.begin(), gameCtx->players.end(), [&](const player_t& alreadyAddedPlayer) {
		    return alreadyAddedPlayer->ID == player->ID ||
		           alreadyAddedPlayer->PrOC->get_color() == player->PrOC->get_color();
	    });
...
game_context_t GameContextBuilder::build(size_t playerCount, rnd_t rnd)
{
	...
	auto playerCountRelatedConfigIt = std::find_if(
	    playerCountRelatedConfigs.begin(), playerCountRelatedConfigs.end(), configByPlayerCount);
	if (playerCountRelatedConfigIt == playerCountRelatedConfigs.end()) {
		throw GameContextBuildingException{"No config provided for the given player count!"};
	}
	...
	std::vector<puoc_etype> puocTypes = add_missing_cards(selectedPuocTypes, *puocContext, rnd);
	std::vector<tc_etype>   tcTypes   = add_missing_cards(selectedTcTypes, *tcContext, rnd);
	...
	tmpCtx->supply = std::make_unique<DiceSupply>(2 * playerCount + 1, clone_dice(rnd), *rnd);

	tmpCtx->numberOfRounds =
	    numberOfRounds.has_value() ? numberOfRounds.value() : defaultNumberOfRounds;
	tmpCtx->dicePerRound = playerCountRelatedConfigIt->diceCount;
	tmpCtx->playerWpcChoiceCount =
	    wpcChoicePerPlayer.has_value() ? wpcChoicePerPlayer.value() : defaultWPCChoicePerPlayer;

	for (auto&& puoc_type : puocTypes) {
		tmpCtx->publicObjectiveCards.push_back(PublicObjectiveCard::create_by_type(
	...
	for (size_t i = 0; i < playerCount; ++i) {
		color_t procColor     = get_unused_proc_color(procIndices, procIndex, tmpCtx);
		proc_t  newPlayerProc = PrivateObjectiveCard::construct(procColor);

		add_player(std::make_unique<PlayerInformation>(i, "", newPlayerProc), tmpCtx);
	}

	if (!tmpCtx->basicMoveHandler) {
		tmpCtx->basicMoveHandler = std::make_unique<BasicMoveHandler>(boardConfig);
	}
...
color_t GameContextBuilder::get_unused_proc_color(std::vector<size_t>& randomizedProcIndices,
                                                  size_t& startingIndex, game_context_t& gameCtx)
{
	for (size_t i = startingIndex; i < randomizedProcIndices.size(); ++i) {
		color_t newColor = color_t(randomizedProcIndices[i]);

		if (std::none_of(
		        gameCtx->players.begin(), gameCtx->players.end(),
		        [&](const player_t& player) { return player->PrOC->get_color() == newColor; })) {
			return newColor;
		}
	}
