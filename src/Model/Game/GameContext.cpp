...
GameContext::GameContext(const GameContext& other, rnd_t _rnd)
    : selectableWPC(other.selectableWPC), numberOfRounds(other.numberOfRounds),
      dicePerRound(other.dicePerRound), playerWpcChoiceCount(other.playerWpcChoiceCount),
      rnd(std::move(_rnd)), supply(other.supply->clone(*rnd)),
      roundTrack(other.roundTrack, *supply, *other.supply), scoreCtx(other.scoreCtx->clone()),
