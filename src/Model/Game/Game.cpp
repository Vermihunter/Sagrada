...
Game::Game(const Game& g, rnd_t rnd)
    : ctx(g.fastctx->clone(std::move(rnd))), fastctx(ctx.get()),
...
Game::Game(const Game& g)
    : ctx(g.fastctx->clone(g.fastctx->rnd->clone())), fastctx(ctx.get()),
      currState(g.currState->clone_identical()), history(g.history)
{
}
...
game_t Game::clone_with_pseudo_random_future(ID_t focusedPlayerID, int seed)
{
	...
	size_t          colorInd      = 0;
	const player_t& focusedPlayer = GameInfo::get_player_with_id(*gameClone, focusedPlayerID);
	for (auto&& player : gameClone->fastctx->players) {
		...
		player->PrOC = std::make_shared<PrivateObjectiveCard>(allColors[colorInd]);
		++colorInd;
	}
...
void Game::basic_move_request(die_to_field_move_t mv, bool isToolCardSubmove)
{
	if (currState->previousToolCardMove && !isToolCardSubmove) {
		...
		throw BadMoveRequest{"Currently the selected tool card has to be used!"};
	}

	/** Throws BadMoveRequest exception if the move is incorrect */
	fastctx->basicMoveHandler->check_move_correct_hardfail(*this, mv, isToolCardSubmove);
...
void Game::remove_die_from_currents(Die* die, bool throwOnNotFound)
{
	...
	if (throwOnNotFound && !diceFound) {
		throw std::runtime_error{"Die not found between the current round's dice!"};
	}

	if (diceFound) {
		currState->currentRoundRemainingDice.erase(diceIt);
		currState->currRoundInfo->turnInfos[currState->currPlayerIndex].dieOriginalInd =
...
{
	if (currState->currRoundInfo->turnInfos[currState->currPlayerIndex].tcUsed &&
	...
	size_t& favorTokensLeft  = GameInfo::get_player_with_id(*this, m->playerId)->favorTokensLeft;
	size_t  favorTokensToUse = fastctx->toolCards[tc_m->ind]->favor_tokens_to_use();
	...
	if (!currState->previousToolCardMove) {
		if (favorTokensLeft < favorTokensToUse) {
			throw BadMoveRequest{"You don't have enough favor tokens to use this tool card!"};
		}
	}

	/** The return value indicates whether it was the last move of the tool card
	 * or not */
	ToolCardUseFeedback tcFeedback = fastctx->toolCards[tc_m->ind]->use(tc_m, *this);
	if (tcFeedback.extractCost) {
		favorTokensLeft -= favorTokensToUse;
	}

	history.moveStack.push_back(MoveInfo{tc_m, currState->previousToolCardMove});
	if (tcFeedback.tcUsageFinished) {
		currState->previousToolCardMove                                        = nullptr;
		currState->currRoundInfo->turnInfos[currState->currPlayerIndex].tcUsed = true;
	}
	else {
...
void Game::plan_next_player_on_move()
{
	...
	if (currState->currPlayerIndex == currState->currRoundInfo->turnInfos.size()) {
...
void Game::plan_player_order()
{
	...
	for (size_t i = 0; i < playerCount; ++i) {
		ID_t nextId =
		    fastctx->players[(i + currState->playerStartedThisRoundIndex) % playerCount]->ID;
...
move_t Game::get_first_possible_move()
{
	...
	move_t firstBasicMove = fastctx->basicMoveHandler->get_first_basic_move(*this, player->ID);
...
move_t Game::get_random_possible_move()
{
	return nullptr;
}
...
move_c Game::possible_moves()
{
	...
	/** basic moves */
	fastctx->basicMoveHandler->append_basic_moves(possibleMoves, *this, player->ID);
...
void Game::undo_dtfm(die_to_field_move_t m, bool putDieBackToCurrRoundsDice, bool shiftRoundsBack)
{
...
void Game::undo_tool_card_move(MoveInfo& m)
{
	...
	ToolCardUseFeedback tcUndoFeedback = fastctx->toolCards[tcm->ind]->undo(tcm, *this);
	...
	if (tcUndoFeedback.tcUsageFinished) {
		currState->currRoundInfo->turnInfos[currState->currPlayerIndex].tcUsed = false;
	}
...
void Game::shift_order_backwards()
{
	...
	if (currState->roundNumber == 0) {
		LOG_E("Shift order backwards called when there were no moves made yet!");
		return;
	}

	--currState->roundNumber;
	currState->playerStartedThisRoundIndex = (currState->playerStartedThisRoundIndex > 0)
...
eval_state_c Game::evaluate()
{
	return EvalState::for_players(*this);
}
