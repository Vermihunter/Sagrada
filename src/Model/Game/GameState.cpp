...
GameState::GameState(const GameState& other, const DiceSupply& from, const DiceSupply& to)
    : hasEnded(other.hasEnded), roundNumber(other.roundNumber),
      playerStartedThisRoundIndex(other.playerStartedThisRoundIndex),
      currPlayerIndex(other.currPlayerIndex), previousToolCardMove(nullptr),
      currRoundInfo(other.currRoundInfo->clone())
{
	for (auto&& die : other.currentRoundRemainingDice) {
		currentRoundRemainingDice.push_back(get_corresponding_die(die, from, to));
	}

	if (other.previousToolCardMove) {
		previousToolCardMove =
		    dynamic_cast<ToolCardMove*>(other.previousToolCardMove->clone(from, to));
	}
}
...
GameState::GameState(const GameState& other)
    : hasEnded(other.hasEnded), roundNumber(other.roundNumber),
      playerStartedThisRoundIndex(other.playerStartedThisRoundIndex),
      currPlayerIndex(other.currPlayerIndex), previousToolCardMove(other.previousToolCardMove),
...
bool GameState::is_curr_player_first_move_this_round()
{
	...
	auto playerIt = std::find_if(
	    currRoundInfo->turnInfos.begin() + currPlayerIndex + 1, currRoundInfo->turnInfos.end(),
	    [&](const TurnInfo& turnInfo) { return turnInfo.playerId == playerId; });
