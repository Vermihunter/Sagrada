 ...
 */
DiceSupply::DiceSupply(size_t _dicePerRound, dice_c _dice,
...
std::vector<Die*> DiceSupply::get_current_round_dice(size_t currentRound)
{
    ...
    if ((currentRound + 1) * dicePerRound > dice->size()) {
        throw std::invalid_argument {
            "There are not enough dice in the supply for round " +
...
Die* DiceSupply::get_unused_die(size_t currentRound)
{
    ...
    auto randomIndexFromUnused =
        rnd.random_in_interval(0, currentlyUnusedDiceCount - 1);
...
Die* get_corresponding_die(Die* die, const DiceSupply& from,
                           const DiceSupply& to)
{
