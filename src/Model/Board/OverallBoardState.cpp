...
void OverallBoardState::add_unfillable_field_indices(const Game& g,
                                                     const ID_t playerID)
{
    auto& bmHandler = g.get_ctx()->basicMoveHandler;
    auto& board = *GameInfo::get_player_with_id(g, playerID)->board;
    const auto placeabilityProcessor =
    ...
    uncompletableFieldCount =
        std::max(passedMoves, unfillableFieldIndices.size());
}