    ...
    : config(other.config), hardness(other.hardness), title(other.title)
{
	...
	for (auto&& row : other.data) {
		for (auto&& field : row) {
			data[rowInd].push_back(field->clone(newDiceSupply, originalDiceSupply));
		}
		++rowInd;
	}
}
...
board_ctx_t BoardContext::clone(DiceSupply& newDiceSupply, DiceSupply& originalDiceSupply)
{
	return std::make_unique<BoardContext>(*this, newDiceSupply, originalDiceSupply);
}