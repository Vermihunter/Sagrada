...
BasicMoveHandler::BasicMoveHandler(const BoardConfig* _boardConfig)
    : boardConfig(_boardConfig)
{
}

bool BasicMoveHandler::check_move_correct_softfail(
    Game& g, const die_to_field_move_t& move, bool allowAnyDie)
{
    return check_move_correct_impl(g, move, allowAnyDie) ==
           basic_move_error_t::NO_ERROR;
}

void BasicMoveHandler::check_move_correct_hardfail(
    Game& g, const die_to_field_move_t& move, bool allowAnyDie)
{
...
basic_move_error_t BasicMoveHandler::check_move_correct_impl(
    Game& g, const die_to_field_move_t& move, bool allowAnyDie)
{
...
bool BasicMoveHandler::die_between_curr_round_dice(Game& g, Die* die)
{
    ...
    return std::find(currRoundDice.begin(), currRoundDice.end(), die) !=
           currRoundDice.end();
}
...
Die* BasicMoveHandler::undo(Game& g, const die_to_field_move_t& move)
{
    ...
    return (*g.get_ctx()->players[playerId]->board)[dtfm->boardIndices]
        ->remove_die();
}

void BasicMoveHandler::append_basic_moves(move_c& possibleMoves, Game& game,
                                          ID_t playerID, dtfm_mode_mask_t mode)
{
    const auto& currentRoundRemainingDice =
        game.get_curr_state()->currentRoundRemainingDice;
    for (auto&& die : currentRoundRemainingDice) {
        for (auto&& ind : indices_to_put_to(game, playerID, die, mode)) {
            possibleMoves.push_back(
                DieToFieldMove::create(playerID, die, ind, mode));
        }
    }
}
...
{
    const auto& currentRoundRemainingDice =
        g.get_curr_state()->currentRoundRemainingDice;
    for (auto&& die : currentRoundRemainingDice) {
        DieToFieldMove putMove {playerId, die, BoardIndex {},
                                DTFM_MODE_DEFAULT};
...
{
    return ind.row == 0 ||
           ind.row == boardConfig->height - 1 || // first or last row
           ind.col == 0 ||
           ind.col == boardConfig->width - 1; // left or right column
}

basic_move_error_t BasicMoveHandler::check_neighbor_colliding(
    Game& g, die_to_field_move_t m)
{
...
{
    for (auto&& neighborInd :
         BoardFieldNeighborCache::GetAllNeighborIndicesFor(ind)) {
...
board_ind_c BasicMoveHandler::indices_to_put_to(Game& g, ID_t playerId,
                                                Die* die, dtfm_mode_mask_t mode)
{
    board_ind_c res;
 ...
 */
basic_move_error_t BasicMoveHandler::check_can_put(board_field_t& field,
                                                   Die* dieToPut,
                                                   dtfm_mode_mask_t mode)
{
    ...
    if (!colorIgnored && colorRestriction &&
        dieToPut->get_color() != colorRestriction.value()) {
        return basic_move_error_t::COLOR_RESTRICTION_ERROR;
    }

    if (!valueIgnored && valueRestriction &&
        dieToPut->get_value() != valueRestriction.value()) {
