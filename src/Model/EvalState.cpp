...
EvalState::EvalState(const player_t& _player, Game& g)
    : player(_player), ctx(g.get_ctx()->scoreCtx), puoc(g.get_ctx()->publicObjectiveCards),
      emptyFieldMinusPoints(0), unusedFavorTokenPoints(0), privateObjectiveCardPoints(0),
      totalPoints(0), boardState(g, player->ID)
...
{
	unusedFavorTokenPoints = player->favorTokensLeft * ctx->pointsPerUnusedFavorToken;
...
{
	emptyFieldMinusPoints =
	    player->board->get_number_of_empty_fields() * ctx->minusPointsPerEmptyField;
...
void EvalState::evaluate_private_objective_card_points()
{
	privateObjectiveCardPoints =
	    player->board->get_total_dice_value_having_color(player->PrOC->get_color()) *
