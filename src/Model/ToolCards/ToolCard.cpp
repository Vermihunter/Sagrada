...
void ToolCard::base_use_impl() { ++useCount; }

void ToolCard::base_undo_impl() { --useCount; }
...
void ToolCard::append_placing_moves(ToolCardAppendContext& ctx, Die* die)
{
    ...
    for (auto&& ind :
         moveHandlerCtx->indices_to_put_to(ctx.g, ctx.playerId, die)) {
        bm.boardIndices = ind;

        ctx.moveContainer.push_back(
            new PlacingDieMove(ctx.playerId, ctx.toolCardInd, bm));
    }
}
    ...
    switch (type) {
        ...
        case tc_etype::MOVE_IGNORE_VAL:
            result = std::make_unique<
                RelocateDieIgnoreAttributeTC<DTFM_MODE_IGNORE_VALUE>>();
            break;
        case tc_etype::MOVE_IGNORE_COLOR:
            result = std::make_unique<
                RelocateDieIgnoreAttributeTC<DTFM_MODE_IGNORE_COLOR>>();
            break;
