...
{
	return g.get_ctx()
	           ->basicMoveHandler->indices_to_put_to(g, rerollSubmove->playerId, rerollSubmove->die)
	           .size() > 0;
}
...
void RerollDieTC::append_reroll_moves(ToolCardAppendContext& ctx)
{
	...
	for (auto&& die : dice) {
		ctx.moveContainer.push_back(
		    new RerollDieTC::Move(ctx.playerId, ctx.toolCardInd, die, die->get_value()));
	}
}
...
ToolCardUseFeedback RerollDieTC::undo(tool_card_move_t& m, Game& g)
{
	...
	base_undo_impl();
	return ToolCardUseFeedback{
	    .shiftRound = !rerollMove->wasPlaceable, .tcUsageFinished = true, .extractCost = true};
}
