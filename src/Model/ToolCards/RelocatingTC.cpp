...
const std::vector<std::string> RelocatingTC::indexErrorStringTable = {
    "", // No error
    "Cannot place two dice to the same field!",
    "Cannot move the same die to two fields!",
    "You have to relocate two different dice!"};

const std::vector<std::string> RelocatingTC::moveErrorStringTable = {
    "", // No error
    "Cannot put a die\nto the same place!",
    "The field doesn't\ncontain a die to move!", "Basic move incorrect!"};

void RelocatingTC::undo_relocate_move(relocate_move_t relocMove, Game& g)
{
    auto& currPlayerBoard =
        *GameInfo::get_player_with_id(g, relocMove->playerId)->board;
...
void RelocatingTC::use_relocate_move(relocate_move_t relocMove, Game& g)
{
    ...
    DieToFieldMove dtfm {playerID, dieToRelocate, relocMove->to,
                         relocMove->mode};
    try {
...
relocating_check_res_t RelocatingTC::check_relocate_move_correct(
    ...
    dtfm_mode_mask_t relocMoveMode, Game& g)
{
    ...
    if (!g.get_ctx()->basicMoveHandler->check_move_correct_softfail(g, &bm,
                                                                    true)) {
...
bool RelocatingTC::check_relocate_move_correct_softfail(
    const BoardIndex& from, const BoardIndex& to, ID_t playerId,
    dtfm_mode_mask_t relocMoveMode, Game& g)
{
...
void RelocatingTC::check_relocate_move_correct_hardfail(
    relocate_move_t relocMove, Game& g)
{
    relocating_check_res_t res =
        check_relocate_move_correct(relocMove->from, relocMove->to,
                                    relocMove->playerId, relocMove->mode, g);
...
void RelocatingTC::append_two_relocate_move_if_correct(
    ToolCardAppendContext& ctx, const RelocateDieMove& m1,
    const RelocateDieMove& m2)
{
    ...
    DieToFieldMove secondRelocateMove {ctx.playerId, move2Die, m2.to,
                                       DTFM_MODE_DEFAULT};
    // We are not actually sending a move request, because its has no additional
    // effect
    if (ctx.g.get_ctx()->basicMoveHandler->check_move_correct_softfail(
            ctx.g, &secondRelocateMove, true)) {
...
{
    return check_indices_correct(mv1, mv2) ==
           relocating_index_check_res_t::IND_OK;
}
...
relocating_index_check_res_t RelocatingTC::check_indices_correct(
    const RelocateDieMove& m1, const RelocateDieMove& m2)
{
    if (m1.to == m2.to) {
