...
ToolCardUseFeedback MoveTwiceInARowTC::use(tool_card_move_t m, Game& g)
{
    if (!g.get_curr_state()->is_curr_player_first_move_this_round()) {
        ...
        throw BadMoveRequest {
            "You can use this card only in your first turn of the round!"};
    }
    ...
    // two assumptions
    auto secondElementIt = std::find_if(
        firstRotateElementIt, currTurns.end(), [&](const TurnInfo& turnInfo) {
            return turnInfo.playerId == currPlayerId;
        });
...
void MoveTwiceInARowTC::append_possible_moves(ToolCardAppendContext& ctx)
{
    ...
    ctx.moveContainer.push_back(
        new MoveTwiceInARowTC::Move(ctx.playerId, ctx.toolCardInd));
}
