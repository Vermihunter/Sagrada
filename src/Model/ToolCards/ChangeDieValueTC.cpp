...
ToolCardUseFeedback ChangeDieValueTC::use(tool_card_move_t m, Game& g)
{
	...
	if (std::find(currRoundsDice.cbegin(), currRoundsDice.cend(), mv->dtfm.die) ==
	    currRoundsDice.cend()) {
		LOG_D("BadMoveRequest 10");
		throw BadMoveRequest{"The die has to be one of the current round's dice!"};
	}
 ...
 */
void ChangeDieValueTC::process_hypothetical_die_with_val(ToolCardAppendContext& ctx, Die* die,
                                                         int changeVal)
{
...
void ChangeDieValueTC::append_moves_for_die(ToolCardAppendContext& ctx, Die* die, int changeVal)
{
	auto possiblePlacingIndices =
	    ctx.g.get_ctx()->basicMoveHandler->indices_to_put_to(ctx.g, ctx.playerId, die);
	for (auto&& possiblePlacingIndex : possiblePlacingIndices) {
		ctx.moveContainer.push_back(
		    ChangeDieValueTC::Move::from_context(ctx, die, changeVal, possiblePlacingIndex));
	}
}
...
tool_card_move_t ChangeDieValueTC::Move::from_context(ToolCardAppendContext& ctx, Die* die,
                                                      int changeVal, const BoardIndex& ind)
{
	return new ChangeDieValueTC::Move(ctx.playerId, ctx.toolCardInd, die, changeVal, ind);
}
