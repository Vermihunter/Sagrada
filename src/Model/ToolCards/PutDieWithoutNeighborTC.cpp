...
ToolCardUseFeedback PutDieWithoutNeighborTC::use(tool_card_move_t m, Game& g)
{
    ...
    if (g.get_ctx()->basicMoveHandler->is_reachable(*pBoard,
                                                    mv->bm.boardIndices)) {
        LOG_D("BadMoveRequest 14");
        throw BadMoveRequest {
            "A move using this tool card has to be unreachable!"};
    }
...
void PutDieWithoutNeighborTC::append_possible_moves(ToolCardAppendContext& ctx)
{
    ...
    DieToFieldMove dummyDtfm = DieToFieldMove {
        ctx.playerId, nullptr, BoardIndex {}, DTFM_MODE_ACCEPT_UNREACHABLE};
    for (auto&& die : ctx.g.get_curr_state()->currentRoundRemainingDice) {
        auto indices = basicMoveHandler->indices_to_put_to(
            ctx.g, ctx.playerId, die, DTFM_MODE_ACCEPT_UNREACHABLE);
        ...
        for (auto&& index : indices) {
            ...
            dummyDtfm.die = die;
            ctx.moveContainer.push_back(new PutDieWithoutNeighborTC::Move(
                ctx.playerId, ctx.toolCardInd, dummyDtfm));
        }
    }
}
...
{
    auto dieWithoutNeighborMove =
        to_concrete_type<PutDieWithoutNeighborTC::Move>(m);
