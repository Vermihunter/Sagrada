...
ToolCardUseFeedback ReplaceDieWithRandomFromBagTC::use(tool_card_move_t m, Game& g)
{
	/**
	...
	if (dieChangingMove) {
		...
		return ToolCardUseFeedback{
		    .shiftRound = !moveExists, .tcUsageFinished = !moveExists, .extractCost = true};
	 ...
	 * it to a field
	 */
	placing_move_t valueChangingMove = to_concrete_type<ValueChangerAndPlacerMove>(m);

	valueChangingMove->placingMove.die->change_value(valueChangingMove->newVal);
	g.move_request(&valueChangingMove->placingMove, true);

	return ToolCardUseFeedback{.shiftRound = true, .tcUsageFinished = true, .extractCost = false};
}

void ReplaceDieWithRandomFromBagTC::replace_die_move_part(replace_move_t m, Game& g)
{
	auto& diceSupply = g.get_ctx()->supply;

	Die* unusedDie = diceSupply->get_unused_die(g.get_curr_state()->roundNumber);
...
bool ReplaceDieWithRandomFromBagTC::move_exists(replace_move_t& replacingMove, Game& g)
{
	...
	for (size_t i = 0; i < size_t(value_t::COUNT); ++i) {
		...
		if (g.get_ctx()
		        ->basicMoveHandler->indices_to_put_to(g, replacingMove->playerId, die)
		        .size() > 0) {
...
void ReplaceDieWithRandomFromBagTC::append_possible_moves(ToolCardAppendContext& ctx)
{
	if (!ctx.prevMove) {
...
void ReplaceDieWithRandomFromBagTC::append_die_changing_moves(ToolCardAppendContext& ctx)
{
	...
	for (auto&& die : ctx.g.get_curr_state()->currentRoundRemainingDice) {
		ctx.moveContainer.push_back(
		    new ReplaceDieWithRandomFromBagTC::DieToChangeMove(ctx.playerId, ctx.toolCardInd, die));
	}
}

void ReplaceDieWithRandomFromBagTC::append_die_placing_moves(ToolCardAppendContext& ctx)
{
	replace_move_t prevDieChangingMove = to_concrete_type<DieToChangeMove>(ctx.prevMove);
	...
	for (size_t i = 0; i < size_t(value_t::COUNT); ++i) {
		...
		auto indicesToPutTo = ctx.g.get_ctx()->basicMoveHandler->indices_to_put_to(
		...
		for (auto&& index : indicesToPutTo) {
			ctx.moveContainer.push_back(new ValueChangerAndPlacerMove(
			    ctx.playerId, ctx.toolCardInd, newVal, originalValue, currentlyUsedDie, index));
		}
	}
...
ToolCardUseFeedback ReplaceDieWithRandomFromBagTC::undo(tool_card_move_t& m, Game& g)
{
	auto valueChangerMove =
	    dynamic_cast<ReplaceDieWithRandomFromBagTC::ValueChangerAndPlacerMove*>(m);

	// This happens first, if the die from the bag was placeable,
	// if it wasn't, this submove didn't happen -> nothing to undo
	if (valueChangerMove) {
		valueChangerMove->placingMove.die->change_value(valueChangerMove->prevVal);
		g.undo_dtfm(&valueChangerMove->placingMove, true, true);

		return ToolCardUseFeedback{
		    .shiftRound = false, .tcUsageFinished = false, .extractCost = false};
	}
