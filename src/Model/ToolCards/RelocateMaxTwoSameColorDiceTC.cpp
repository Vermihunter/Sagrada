...
ToolCardUseFeedback RelocateMaxTwoSameColorDiceTC::use(tool_card_move_t m,
                                                       Game& g)
{
    auto mv = to_concrete_type<RelocateTwoDiceMove>(m);
    auto& currPlayerBoard =
        GameInfo::get_player_with_id(g, mv->playerId)->board;
...
void RelocateMaxTwoSameColorDiceTC::append_possible_moves(
    ToolCardAppendContext& ctx)
{
    std::array<bool, Color::SIZE> roundTrackContainsColors;
    for (size_t i = 0; i < Color::SIZE; ++i) {
        roundTrackContainsColors[i] =
            round_track_contains_color(ctx.g, color_t(i));
    }
...
bool RelocateMaxTwoSameColorDiceTC::round_track_contains_color(Game& g,
                                                               color_t color)
{
    return g.get_ctx()->roundTrack.contains_die_with_color(color);
}

ToolCardUseFeedback RelocateMaxTwoSameColorDiceTC::undo(tool_card_move_t& m,
                                                        Game& g)
{
