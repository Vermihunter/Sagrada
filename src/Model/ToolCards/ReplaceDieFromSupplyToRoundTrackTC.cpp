...
ToolCardUseFeedback ReplaceDieFromSupplyToRoundTrackTC::use(tool_card_move_t m,
                                                            Game& g)
{
...
void ReplaceDieFromSupplyToRoundTrackTC::replace_dice(replace_move_t& mv,
                                                      Game& g)
{
...
void ReplaceDieFromSupplyToRoundTrackTC::append_possible_moves(
...
void ReplaceDieFromSupplyToRoundTrackTC::append_moves_for(
    Die* currentRoundDie, Die* roundTrackDie, ToolCardAppendContext& ctx)
{
    auto& basicMoveHandler = ctx.g.get_ctx()->basicMoveHandler;
    DieToFieldMove placeMove {ctx.playerId, roundTrackDie, BoardIndex {},
                              DTFM_MODE_DEFAULT};
    for (auto&& possibleBoardInd : basicMoveHandler->indices_to_put_to(
             ctx.g, ctx.playerId, roundTrackDie)) {
        ...
        ctx.moveContainer.push_back(
            new ReplaceDieFromSupplyToRoundTrackTC::Move(
                ctx.playerId, ctx.toolCardInd, currentRoundDie, roundTrackDie,
                placeMove));
    }
}

ToolCardUseFeedback ReplaceDieFromSupplyToRoundTrackTC::undo(
    tool_card_move_t& m, Game& g)
{
    auto replaceMove =
        to_concrete_type<ReplaceDieFromSupplyToRoundTrackTC::Move>(m);
