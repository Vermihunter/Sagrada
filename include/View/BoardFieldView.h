...
class BoardFieldView
{
  public:
     ...
     */
    BoardFieldView(const Point& upperLeft, const SizeInPixels& size,
    ...
    void set_selected(Color* strokeColor = &black)
    {
  ...
  private:
     ...
     */
    void draw_circle(const Cairo::RefPtr<Cairo::Context>& cr,
                     size_t centerXInPixels, size_t centerYInPixels,
                     size_t radiusInPixels, Color& fill_color);

    void draw_value_circles(const Cairo::RefPtr<Cairo::Context>& cr,
