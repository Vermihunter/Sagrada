...
class CurrentRoundsDiceView : public Gtk::DrawingArea, public IDieClickSignaller {
  public:
	...
	CurrentRoundsDiceView(const SizeInPixels& _fieldSize, const std::vector<Die*>& _currRoundDice)
	    : fieldSize(_fieldSize), currRoundDice(_currRoundDice)
	{
		size_t rowCount =
		    (currRoundDice.size() / dicePerRow) + (currRoundDice.size() % dicePerRow != 0 ? 1 : 0);
		...
		for (auto&& die : currRoundDice) {
			...
			diceView.push_back(std::make_unique<BoardFieldView>(
			    Point{.x = leftAndRightMargin +
			               (ind % dicePerRow) * (fieldSize.width + spaceBetweenDice),
			          .y = topAndBottomMargin +
			               (ind / dicePerRow) * (fieldSize.height + spaceBetweenDice)},
			    fieldSize, diceWrapper.back()));
			++ind;
		}

		add_events(Gdk::BUTTON_PRESS_MASK);
		size.width = 2 * leftAndRightMargin + std::min(dicePerRow, currRoundDice.size()) *
		                                          (fieldSize.width + spaceBetweenDice);
		size.height = 2 * topAndBottomMargin + rowCount * (fieldSize.height + spaceBetweenDice);
  ...
  protected:
	virtual bool on_button_press_event(GdkEventButton* e)
	{
		for (auto&& dieView : diceView) {
			...
			if (upperLeft.x <= e->x && upperLeft.x + size.width >= e->x && upperLeft.y <= e->y &&
			    upperLeft.y + size.height >= e->y) {
				signal_with_die(die);
				break;
			}
		}
	...
	virtual bool on_draw(const Cairo::RefPtr<Cairo::Context>& cr) override
	{
		for (auto&& dieWrapper : diceWrapper) {
			...
			auto dieIt = std::find(currRoundDice.begin(), currRoundDice.end(), die);
