...
class RelocatingToolCardView : public ToolCardView {
  public:
	                                              ...
	                                              const BoardIndex&     boardInd) override
	{
		...
		auto secondRelocateMove =
		    std::make_shared<RelocateDieMove>(playerOnMoveID, ind, *fromField, boardInd);
		auto twoRelocateMoves = new RelocateTwoDiceMove(*firstRelocateMove, *secondRelocateMove);
		swap_at_ind(firstRelocateMove->from, firstRelocateMove->to);
		firstRelocateMove = nullptr;

		signalProcessor->send_move_request(twoRelocateMoves, moveResultHandlingInfo);
	}
  ...
  protected:
	...
	RelocatingToolCardView(ToolCardViewContainer& parentContainer, Game& _game, ToolCard& _tc,
	                       const SizeInPixels& size, size_t _ind)
	    : ToolCardView(parentContainer, _game, _tc, size, _ind), tc(_tc),
	      firstRelocateMove(nullptr), usedBoardView(nullptr)
	{
	}
	...
	move_c try_make_ind_from_field(ToolCard& concreteToolCard, RectangularBoardView* boardView,
	                               const BoardIndex& boardInd, tool_card_move_t prevMove)
	{
		Die* fromFieldDie = boardView->get_ctx().data[boardInd.row][boardInd.col]->get_die();
	                                      ...
	                                      const BoardIndex&     boardInd)
	{
		...
		auto usableFirstRelocateMoveIt =
		    std::find_if(cachedMoves.begin(), cachedMoves.end(), searchRelatedRelocMoveFields);
		if (usableFirstRelocateMoveIt == cachedMoves.end()) {
			return signalProcessor->show_error_message("You cannot move that die there!");
		}

		firstRelocateMove = std::make_unique<RelocateDieMove>(
		    game.get_curr_state()->player_on_move_id(), ind, *fromField, boardInd);
	...
	void process_from_field(RectangularBoardView* boardView, const BoardIndex& boardInd)
	{
		if (!firstRelocateMove) {
			cachedMoves.clear();
			cachedMoves = try_make_ind_from_field(tc, boardView, boardInd, nullptr);
		}
		...
		for (auto&& move : cachedMoves) {
			...
			auto& relocMove    = (firstRelocateMove) ? twoRelocMove->m2 : twoRelocMove->m1;
			...
			if (firstRelocateMove && (twoRelocMove->m1.from != firstRelocateMove->from ||
		...
		signalProcessor->highlight_fields(placeableIndices,
