...
class PutDieWithoutNeighborTCView : public ToolCardView {
  public:
	PutDieWithoutNeighborTCView(ToolCardViewContainer& _parentContainer, Game& game, ToolCard& _tc,
	                            const SizeInPixels& size, size_t ind)
	    : ToolCardView(_parentContainer, game, _tc, size, ind), tc((PutDieWithoutNeighborTC&)(_tc)),
	      selectedDie(nullptr)
	{
	}
	...
	virtual void receive_die_click_signal(Die* die)
	{
		...
		ToolCardAppendContext tmpCtx{tmpMoveContainer, game, ind, playerOnMoveId, nullptr};
		tc.append_possible_moves(tmpCtx);

		board_ind_c selectableBoardIndices;
		for (auto&& move : tmpMoveContainer) {
			PutDieWithoutNeighborTC::Move* castedMove = (PutDieWithoutNeighborTC::Move*)(move);
		...
		signalProcessor->highlight_fields(selectableBoardIndices, playerOnMoveId);
	}
	...
	virtual void receive_board_field_click_signal(RectangularBoardView*, const BoardIndex& boardInd)
	{
		...
		DieToFieldMove dtfm{game.get_curr_state()->player_on_move_id(), selectedDie, boardInd,
		                    DTFM_MODE_ACCEPT_UNREACHABLE};
		auto newMove = new PutDieWithoutNeighborTC::Move(game.get_curr_state()->player_on_move_id(),
		                                                 ind, dtfm);
