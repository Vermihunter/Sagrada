...
class RerollDieTCView : public ToolCardView {
  public:
	RerollDieTCView(ToolCardViewContainer& _parentContainer, Game& game, ToolCard& _tc,
	                const SizeInPixels& size, size_t ind)
	    : ToolCardView(_parentContainer, game, _tc, size, ind), tc((RerollDieTC&)(_tc)),
	      selectedDie(nullptr)
	{
	}
	...
	virtual void receive_die_click_signal(Die* die)
	{
		...
		auto move =
		    new RerollDieTC::Move(playerOnMoveId, ind, selectedDie, selectedDie->get_value());
		auto       clonedMove = move->clone(*game.get_ctx()->supply, *game.get_ctx()->supply);
		const bool moveSuccessful =
		    signalProcessor->send_move_request(move, rerollMoveResultHandlingInfo);
		...
		ToolCardAppendContext tmpCtx{tmpMoveContainer, game, ind, playerOnMoveId,
		                             tool_card_move_t(clonedMove)};
		tc.append_possible_moves(tmpCtx);

		if (tmpMoveContainer.empty()) {
			signalProcessor->show_error_message("Unfortunately you cannot place the rerolled die!");
		...
		std::transform(tmpMoveContainer.begin(), tmpMoveContainer.end(),
		               ...
		               [](const move_t& m) { return ((PlacingDieMove*)(m))->bm.boardIndices; });
	...
	void receive_board_field_click_signal(RectangularBoardView*, const BoardIndex& boardInd)
	{
		if (!selectedDie) {
		...
		ID_t playerOnMoveId = game.get_curr_state()->player_on_move_id();
		auto placingMove    = new PlacingDieMove(
		       playerOnMoveId, ind,
		       DieToFieldMove{playerOnMoveId, selectedDie, boardInd, DTFM_MODE_DEFAULT});
		const bool moveSuccessful =
		    signalProcessor->send_move_request(placingMove, placingMoveResultHandlingInfo);
  ...
  private:
	...
	constexpr static const MoveResultHandlingPolicy placingMoveResultHandlingInfo{
	    .deselectOnFailure = false, .deselectOnSuccess = true};

	constexpr static const MoveResultHandlingPolicy rerollMoveResultHandlingInfo{
