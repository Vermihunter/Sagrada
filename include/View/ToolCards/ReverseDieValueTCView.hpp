...
{
  public:
    ReverseDieValueTCView(ToolCardViewContainer& _parentContainer, Game& game,
    ...
    virtual void receive_die_click_signal(Die* die) override
    {
        ...
        board_ind_c reversedDieValuePlaceableIndices =
            game.get_ctx()->basicMoveHandler->indices_to_put_to(
                game, playerOnMoveId, selectedDie);
        ...
        signalProcessor->highlight_fields(reversedDieValuePlaceableIndices,
                                          playerOnMoveId);
    }

    virtual void receive_board_field_click_signal(
        RectangularBoardView* boardView, const BoardIndex& boardInd)
    {
        if (!selectedDie) {
            return signalProcessor->show_error_message(
        ...
        auto move = new PlacingDieMove(
            playerID, ind,
            DieToFieldMove {playerID, tmpDie, boardInd, DTFM_MODE_DEFAULT});
