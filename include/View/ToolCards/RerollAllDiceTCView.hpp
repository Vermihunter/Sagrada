...
class RerollAllDiceTCView : public ToolCardView {
  public:
	RerollAllDiceTCView(ToolCardViewContainer& _parentContainer, Game& game, ToolCard& tc,
	                    const SizeInPixels& size, size_t ind)
	    : ToolCardView(_parentContainer, game, tc, size, ind), moveInProgress(false)
	{
	}
	...
	virtual bool set_selected(bool _isSelected) override
	{
		...
		moveInProgress = true;
		auto move      = new RerollAllDiceTC::Move(game.get_curr_state()->player_on_move_id(), ind);
