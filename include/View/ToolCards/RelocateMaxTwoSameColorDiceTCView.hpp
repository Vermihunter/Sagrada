...
class RelocateMaxTwoSameColorDiceTCView : public RelocatingToolCardView {
  public:
	RelocateMaxTwoSameColorDiceTCView(ToolCardViewContainer& _parentContainer, Game& game,
	                                  ToolCard& tc, const SizeInPixels& size, size_t ind)
	    : RelocatingToolCardView(_parentContainer, game, tc, size, ind), size(550, 70)
	{
	}
  ...
  private:
	...
	void draw_perimeter(const Cairo::RefPtr<Cairo::Context>& cr, const Color& color, size_t param,
	                    size_t lineWidth, size_t ind)
	{
