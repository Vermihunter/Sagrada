...
{
  public:
    ReplaceDieWithRandomFromBagTCView(ToolCardViewContainer& _parentContainer,
    ...
    virtual void receive_board_field_click_signal(
        RectangularBoardView*, const BoardIndex& boardInd) override
    {
        if (!selectedDie) {
        ...
        const bool moveSuccessful = signalProcessor->send_move_request(
            &move, diePlacingMoveResultHandlingInfo);
    ...
    virtual void receive_die_click_signal(Die* die) override
    {
        ...
        ID_t playerOnMoveID = game.get_curr_state()->player_on_move_id();
        auto move = new ReplaceDieWithRandomFromBagTC::DieToChangeMove(
            playerOnMoveID, ind, selectedDie);
        bool existsMove = fill_cache_for_move(move);
        const bool moveSuccessful = signalProcessor->send_move_request(
            move, bagPlaceMoveResultHandlingInfo);
  ...
  private:
    struct ValueContext
    {
        ValueContext(value_t _val, size_t ind)
            : boardFieldDie(color_t::RED, _val)
        {
            value = _val;
            // boardField = BoardField::create_empty();
            boardField.put_die(&boardFieldDie);
            boardFieldView = std::make_unique<BoardFieldView>(
                Point {10 + ind * (50 + 7), 10}, SizeInPixels {50, 50},
                boardField);
        }
    ...
    constexpr static const MoveResultHandlingPolicy
        bagPlaceMoveResultHandlingInfo {.deselectOnFailure = true,
                                        .deselectOnSuccess = false};

    constexpr static const MoveResultHandlingPolicy
        diePlacingMoveResultHandlingInfo {.deselectOnFailure = false,
     ...
     */
    bool fill_cache_for_move(
        ReplaceDieWithRandomFromBagTC::DieToChangeMove* prevMove)
    {
        move_c tmpMoveContainer;
        ToolCardAppendContext tmpAppendCtx {tmpMoveContainer, game, ind,
                                            prevMove->playerId, prevMove};
        reset_all_value_contexts();

        tc.append_possible_moves(tmpAppendCtx);

        for (auto&& move : tmpMoveContainer) {
            auto castedMove = (ReplaceDieWithRandomFromBagTC::
                                   ValueChangerAndPlacerMove*)(move);
            valueContexts[size_t(castedMove->newVal)]->usableIndices.push_back(
                castedMove->placingMove.boardIndices);
        }

        return std::any_of(
            valueContexts.begin(), valueContexts.end(),
