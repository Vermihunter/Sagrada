...
class MoveTwiceInARowTCView : public ToolCardView {
  public:
	MoveTwiceInARowTCView(ToolCardViewContainer& _parentContainer, Game& game, ToolCard& _tc,
	                      const SizeInPixels& size, size_t ind)
	    : ToolCardView(_parentContainer, game, _tc, size, ind), tc((MoveTwiceInARowTC&)(_tc)),
	...
	virtual bool set_selected(bool _isSelected) override
	{
		...
		moveInProgress = true;
		auto move = new MoveTwiceInARowTC::Move(game.get_curr_state()->player_on_move_id(), ind);
