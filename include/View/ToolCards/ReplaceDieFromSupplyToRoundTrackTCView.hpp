...
{
  public:
    ReplaceDieFromSupplyToRoundTrackTCView(
        ToolCardViewContainer& _parentContainer, Game& game, ToolCard& tc,
    ...
    virtual void receive_round_track_die_click_signal(
        Die* roundTrackDie) override
    {
        if (!currSupplyDie) {
        ...
        board_ind_c placeableIndices =
            game.get_ctx()->basicMoveHandler->indices_to_put_to(
                                                  ...
                                                  const BoardIndex& boardInd)
    {
        ...
        DieToFieldMove dtfm {playerOnMoveId, currSupplyDie, boardInd,
                             DTFM_MODE_DEFAULT};
        auto move = new ReplaceDieFromSupplyToRoundTrackTC::Move(
            playerOnMoveId, ind, currSupplyDie, currRoundTrackDie, dtfm);
