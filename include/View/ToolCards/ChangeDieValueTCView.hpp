...
class ChangeDieValueTCView : public ToolCardView
{
  public:
        ...
        RectangularBoardView*, const BoardIndex& boardInd) override
    {
        ...
        auto move = new ChangeDieValueTC::Move(playerOnMoveID, ind, selectedDie,
                                               selectedValue, boardInd);
  ...
  private:
    ...
    {
        ValueBasedContext(const Point& upperLeft, const SizeInPixels& fieldSize,
                          int _value)
            : underlyingDie(color_t::RED, value_t::ONE), value(_value)
        {
            ...
            fieldView = std::make_unique<BoardFieldView>(upperLeft, fieldSize,
                                                         *underlyingField);
        }
    ...
    void visualize_context(const Cairo::RefPtr<Cairo::Context>& cr,
                           ValueBasedContext& valueContext)
    {
        ...
        if (selectedValue == 0) {
            value_t newValue =
                value_t(size_t(selectedDie->get_value()) + valueContext.value);
    ...
    bool check_click_for_context(GdkEventButton* event,
                                 ValueBasedContext& valueContext)
    {
        if (valueContext.cachedValueIndices.empty()) {
            return false;
        ...
        if (!valueContext.fieldView->contains_point(
        ...
        signalProcessor->highlight_fields(
            valueContext.cachedValueIndices,
    ...
    void fill_cache_for_die(Die* die)
    {
        ...
        ToolCardAppendContext tcAppendCtx {
            tmpMoveContainer, game, ind,
        ...
        for (auto&& move : tmpMoveContainer) {
            ChangeDieValueTC::Move* castedMove =
                (ChangeDieValueTC::Move*)(move);

            if (castedMove->dtfm.die == die) {
                if (castedMove->changeVal == 1) {
                    plusOneValueContext.cachedValueIndices.push_back(
