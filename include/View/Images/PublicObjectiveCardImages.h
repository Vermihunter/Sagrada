...
struct PublicObjectiveCardImages {
	...
	static void load()
	{
		instance = std::unique_ptr<PublicObjectiveCardImages>(new PublicObjectiveCardImages());
	}
	...
	PublicObjectiveCardImages()
	{
		for (size_t i = 0; i < size_t(puoc_etype::COUNT); ++i) {
			images.push_back(std::make_unique<Image>(
			    ...
			    SizeInPixels{defaulCardtWidth, size_t(defaulCardtWidth / puocWidthToHeightRatio)}));
		}
	}

	PublicObjectiveCardImages(const PublicObjectiveCardImages&)            = delete;
	PublicObjectiveCardImages& operator=(const PublicObjectiveCardImages&) = delete;
