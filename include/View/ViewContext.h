...
struct ViewContext
{
    ...
    ViewContext with_reduced_width(double factor)
    {
        return with_reduced_size(factor, 1.0);
    }

    ViewContext with_reduced_height(double factor)
    {
        return with_reduced_size(1.0, factor);
    }

    ViewContext with_reduced_size(double widthFactor, double heightFactor)
    {
        return ViewContext {SizeInPixels {size_t(size.width * widthFactor),
                                          size_t(size.height * heightFactor)},
