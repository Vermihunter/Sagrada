...
class CardIcon : public Gtk::EventBox {
  public:
	CardIcon(const std::string& iconPath, const SizeInPixels& size, Gtk::Box& _cards,
	         const std::string& tooltipText, Gtk::Box*& _currentlyShownCards)
	    : cards(_cards), currentlyShownCards(_currentlyShownCards), icon(iconPath, size)
	{
		add(icon);
	...
	{
		return std::make_unique<CardIcon>(v_constants::puocIconPath, size, cards,
		                                  "Public objective cards", currentlyShownCards);
	}

	static auto create_tc_icon(const SizeInPixels& size, Gtk::VBox& cards,
	                           Gtk::Box*& currentlyShownCards)
	{
		return std::make_unique<CardIcon>(v_constants::tcIconPath, size, cards, "Tool cards",
		                                  currentlyShownCards);
	}
	                             ...
	                             Gtk::Box*& currentlyShownCards)
	{
		return std::make_unique<CardIcon>(v_constants::procIconPath, size, card,
		                                  "Private objective card", currentlyShownCards);
	}
