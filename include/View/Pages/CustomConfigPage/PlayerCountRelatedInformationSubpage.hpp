...
class PlayerCountRelatedInformationSubpage : public CustomConfigPageSubpage {
  public:
	PlayerCountRelatedInformationSubpage(const ViewContext& ctx)
	    : CustomConfigPageSubpage(ctx), lne_PlayerCount("Enter player count"),
	      lne_DiceCount("Enter dice count per round"), hbox_TitleLabels(true),
	      label_PlayerCount("Player count"), label_DicePerRound("Dice per round"),
	      label_Settings("Settings"), label_Delete("Delete"),
	      button_AddNew("+ Add new configuration")

	{
		button_AddNew.signal_clicked().connect(
		    sigc::mem_fun(*this, &PlayerCountRelatedInformationSubpage::add_new_button_clicked));
	...
	{
		auto configTransformer =
		    std::views::transform([](const std::unique_ptr<PlayerCountRelatedConfig>& obj) {
			    return PlayerCountRelatedGameInformation{.playerCount = obj->get_player_count(),
			                                             .diceCount =
			                                                 obj->get_dice_per_round_count()};
		    });

		std::vector<PlayerCountRelatedGameInformation> enteredConfigs;

		std::ranges::copy(configs | configTransformer, std::back_inserter(enteredConfigs));
  ...
  private:
	...
	void add_config(size_t playerCount, size_t diceCount)
	{
		...
		if (std::find_if(configs.begin(), configs.end(), configByPlayerCount) != configs.end()) {
			// display error message
			reset_attributes();
			return;
		}
		...
		auto newConfig = std::make_unique<PlayerCountRelatedConfig>(playerCount, diceCount);
		newConfig->delete_button().signal_clicked().connect(sigc::bind(
		    sigc::mem_fun(*this,
		                  &PlayerCountRelatedInformationSubpage::config_delete_button_clicked),
		    newConfig.get()));

		newConfig->settings_button().signal_clicked().connect(sigc::bind(
		    sigc::mem_fun(*this,
		                  &PlayerCountRelatedInformationSubpage::config_settings_button_clicked),
		    newConfig.get()));
	...
	void config_delete_button_clicked(PlayerCountRelatedConfig* sender)
	{
		...
		const auto findByRaw = [&](const std::unique_ptr<PlayerCountRelatedConfig>& config) {
			return config.get() == sender;
		};

		configs.erase(std::find_if(configs.cbegin(), configs.cend(), findByRaw));
		if (configs.empty()) {
			hbox_TitleLabels.hide();
		}
	}
	...
	{
		lne_PlayerCount.widget.set_text(std::to_string(sender->get_player_count()));
		lne_DiceCount.widget.set_text(std::to_string(sender->get_dice_per_round_count()));
