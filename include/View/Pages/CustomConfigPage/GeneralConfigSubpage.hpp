...
class GeneralConfigSubpage : public CustomConfigPageSubpage
{
  public:
    GeneralConfigSubpage(const ViewContext& ctx)
        ...
        , lEntry_contextName("Game mode name")
        , lEntry_numberOfRounds("Number of rounds",
                                std::to_string(defaultNumberOfRounds))
    ...
    general_config_subpage_info_t collect_information()
    {
        return std::make_unique<GeneralConfigSubpageInformation>(
            ...
            lEntry_numberOfRounds.widget.get_underlying_number(
                "Number of rounds"),
            lEntry_wpcChoiceForPlayers.widget.get_underlying_number(
                "WPC choice per player"));
    }
