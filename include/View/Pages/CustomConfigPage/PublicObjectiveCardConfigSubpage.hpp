...
class PublicObjectiveCardConfigSubpage : public CustomConfigPageSubpage {
  ...
  public:
	PublicObjectiveCardConfigSubpage(const ViewContext& ctx)
	    : CustomConfigPageSubpage(ctx), lne_CardsPerGame("Cards per game"),
	      alignment_CardPerGameMiddle(0.5, 0, 0), alignment_CardPacksMiddle(0.5, 0, 0)
	{
		...
		for (auto&& puocImage : PublicObjectiveCardImages::get_all()) {
			...
			puocConfigs.push_back(std::make_unique<PublicObjectiveCardConfig>(
			    *puocImage, type, defaultPuocCtx->get_context_for(type)->get_satisfaction_value()));

			if (ind < configPerRow) {
				hbox_CardsPack1.pack_start(*puocConfigs.back(), Gtk::PACK_SHRINK, 5);
			}
			else {
				hbox_CardsPack2.pack_start(*puocConfigs.back(), Gtk::PACK_SHRINK, 5);
			}
			++ind;
		}
	...
	{
		auto selectedFilter =
		    std::views::filter([](const std::unique_ptr<PublicObjectiveCardConfig>& obj) {
			    return obj->is_selected();
		    });

		auto puocConfigGetter =
		    std::views::transform([](const std::unique_ptr<PublicObjectiveCardConfig>& obj) {
			    return std::make_pair(
			        obj->type,
			        obj->lne_SatisfactionValue.widget.get_underlying_number("Satisfaction value"));
		    });
