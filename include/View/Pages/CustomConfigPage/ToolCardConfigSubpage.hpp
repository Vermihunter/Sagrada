  ...
  public:
        ...
        , alignment_CardPerGameMiddle(0.5, 0, 0)
    {
        ...
        for (auto&& tcImage : ToolCardImages::get_all()) {
            tcConfigs.push_back(
                std::make_unique<ToolCardConfig>(*tcImage, tc_etype(ind)));

            if (ind < configPerRow) {
                hbox_CardsPack1.pack_start(*tcConfigs.back(), Gtk::PACK_SHRINK,
                                           5);
            }
            else {
                hbox_CardsPack2.pack_start(*tcConfigs.back(), Gtk::PACK_SHRINK,
                                           5);
            }
    ...
    {
        auto selectedFilter =
            std::views::filter([](const std::unique_ptr<ToolCardConfig>& obj) {
                return obj->is_selected();
            });

        auto tcConfigGetter = std::views::transform(
            [](const std::unique_ptr<ToolCardConfig>& obj) {
                return obj->type;
            });
