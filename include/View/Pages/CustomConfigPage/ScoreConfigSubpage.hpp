  ...
  public:
	ScoreConfigSubpage(const ViewContext& ctx)
	    : CustomConfigPageSubpage(ctx),
	      ...
	      lEntry_procValuePoints("Points for each die value of Private objective card color"),
	      vbox_ScoreSettings(true)
	{
		...
		lEntry_procValuePoints.widget.set_text(std::to_string(defaultScoreCtx->pointsPerProcValue));
	...
	score_ctx_t get_entered_score_config()
	{
		return std::make_unique<ScoreContext>(
		    lEntry_unusedFavorTokenPoints.widget.get_underlying_number("Unused favor token points"),
		    lEntry_minusPointsPerEmptyField.widget.get_underlying_number(
		        "Minus points for empty fields"),
		    lEntry_procValuePoints.widget.get_underlying_number("PROC value points"));
	}
