...
struct CommonGameElements {
	...
	static auto create(Game& g, const std::vector<ID_t> localPlayerIds, size_t cardWidth)
	{
		...
		for (auto&& gamePuoc : g.get_ctx()->publicObjectiveCards) {
			...
			instance->puocCards.push_back(std::make_unique<Image>(
			    path, SizeInPixels{cardWidth, size_t(cardWidth / puocWidthToHeightRatio)}));
			instance->puocViews.pack_start(*instance->puocCards.back(), Gtk::PACK_EXPAND_PADDING,
			                               10);
		}
		...
		for (auto&& localPlayerID : localPlayerIds) {
			...
			instance->procViews.pack_start(*instance->localPlayerProcCards.back());
		}
