...
class MCTSAI_TypeSpecifierView : public AI_TypeSpecifierView {
  public:
	MCTSAI_TypeSpecifierView()
	    : iterationCountSpecifier("Iterations"), playoutStrategySpecifier("Playout strategy"),
	      worldCountSpecifier("World count")
	{
		for (auto&& predefinedIterationCount : predefinedIterationCounts) {
			iterationCountSpecifier.widget.append(std::to_string(predefinedIterationCount));
		}
	...
	virtual ai_player_config_t create_player() override
	{
		...
		ctx->iterationCount =
		    predefinedIterationCounts[iterationCountSpecifier.widget.get_active_row_number()];
		ctx->determinizingWorldCount = worldCountSpecifier.widget.get_active_row_number() + 1;

		switch (playoutStrategySpecifier.widget.get_active_row_number()) {
		case 0:
			ctx->playoutStrategy = std::make_unique<MinimaxPlayerConfig>(1, defaultConstants,
			                                                             MINIMAX_defaultWorldCount);
			break;
		case 1:
			ctx->playoutStrategy =
			    std::make_unique<RulesBasedPlayerConfig>(RulesBasedPlayer::strategy_t::ONLY_DTFM);
			break;
		case 2:
			ctx->playoutStrategy =
			    std::make_unique<RulesBasedPlayerConfig>(RulesBasedPlayer::strategy_t::ALL_MOVES);
			break;
		case 3:
  ...
  private:
	inline const static std::vector<int> predefinedIterationCounts{10,  20,  30,  50, 100,
	                                                               150, 200, 250, 300};
