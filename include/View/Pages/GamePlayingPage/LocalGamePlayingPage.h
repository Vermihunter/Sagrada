...
class LocalGamePlayingPage : public GamePlayingPage,
  ...
  public:
	LocalGamePlayingPage(const ViewContext& ctx, local_player_controller_t _controller,
	...
	virtual void receive_board_field_click_signal(RectangularBoardView* board,
	                                              const BoardIndex&     ind) override;
	virtual void tool_card_clicked(ToolCardView* clickedTcView, bool forceSelect = false) override;
	virtual bool send_move_request(move_t m, const MoveResultHandlingPolicy&) override;
	...
	virtual void highlight_fields(const board_ind_c& indices, ID_t playerID) override;
	virtual void redraw_curr_rounds_dice() override;
	virtual void block_features(const BlockedUserInterraction& newState) override;
  ...
  private:
	...
	constexpr static const BlockedUserInterraction defaultState{.blockedTcClick         = false,
