...
struct GamePlayingPageState {
  private:
	...
	{
		return std::find_if(
		    ctx.currRoundsDice.diceView.begin(), ctx.currRoundsDice.diceView.end(),
		    [&](const auto& dieView) { return dieView->get_field().get_die() == die; });
	}
  ...
  public:
	GamePlayingPageState(GamePlayingPageContext& _ctx, const BlockedUserInterraction& startingState)
	    : selectedDie(nullptr), selectedTc(nullptr), blockState(startingState), ctx(_ctx)
	{
	}
