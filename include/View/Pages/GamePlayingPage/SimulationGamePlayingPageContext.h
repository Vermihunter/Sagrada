...
struct SimulationGamePlayingPageContext : public GamePlayingPageContext {
	SimulationGamePlayingPageContext(const ViewContext& viewCtx, ControllerWithAIPlayer& controller)
	    : GamePlayingPageContext(
	          viewCtx, controller,
	          CommonGameElements::create(controller.get_game(), controller.get_all_ids(), 155)),
	      button_Next("Next")
	{
