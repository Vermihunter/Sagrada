...
class Logger {
  public:
	...
	template <HasOutputOperator T> void log(log_level level, const T& msg)
	{
		...
		/** Log format */
		(*config.logStream) << "[ " << logStringByLevel.at(level) << " - " << Date::get_curr_date()
		                    << " - " << Date::get_curr_time() << " ] - " << msg << std::endl;
	}
...
inline log_level log_level_from_str(std::string str)
{
	...
	auto it = std::find_if(Logger::logStringByLevel.begin(), Logger::logStringByLevel.end(),
...
template <HasOutputOperator T> inline void LOG_T(const T& logMsg)
{
	Logger::get_instance().log(log_level::TRACE, logMsg);
}

template <HasOutputOperator T> inline void LOG_D(const T& logMsg)
{
	Logger::get_instance().log(log_level::DEBUG, logMsg);
}

template <HasOutputOperator T> inline void LOG_I(const T& logMsg)
{
	Logger::get_instance().log(log_level::INFO, logMsg);
}

template <HasOutputOperator T> inline void LOG_W(const T& logMsg)
{
	Logger::get_instance().log(log_level::WARNING, logMsg);
}

template <HasOutputOperator T> inline void LOG_E(const T& logMsg)
{
	Logger::get_instance().log(log_level::ERROR_, logMsg);
}

template <HasOutputOperator T> inline void LOG_F(const T& logMsg)
{
