...
template <typename T>
struct MinimizingPlayer : MinimaxPlayerType<T>
{
    virtual T get_initial_value() override { return MinMaxVal<T>::max; }

    virtual bool is_better_value(T currVal, T currBest) override
    {
        return currVal < currBest;
    }
...
template <typename T>
struct MaximizingPlayer : MinimaxPlayerType<T>
{
