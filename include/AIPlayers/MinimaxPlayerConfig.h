...
struct MinimaxPlayerConfig : public AI_PlayerConfig {
  public:
	...
	MinimaxPlayerConfig(size_t _depth, const HeuristicConstants& _constants, int _worldCount);

	virtual ai_player_t create_player(const AIPlayerContext& _ctx) const override;

	virtual const std::string& get_name() const override { return nameWithDepth; }
