...
class MinimaxPlayer : public AI_Player {
  public:
	MinimaxPlayer(const AIPlayerContext& _ctx, size_t depth, const HeuristicConstants& _constants,
	              int worldCount);
  ...
  private:
	 ...
	 */
	heuristic_result_t get_heuristic_value_for_score(const eval_state_t& evalState);
	 ...
	 */
	std::pair<move_t, heuristic_result_t>
	minimax_algorithm(Game& g, size_t depth, int alpha, int beta,
	 ...
	 */
	move_by_index_c& get_moves(Game& g, move_c& allMoves, move_by_index_c& bestMoves,
