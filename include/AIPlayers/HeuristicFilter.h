...
inline bool filter_die_to_field_move(Board& board, DieToFieldMove& move)
{
	for (auto&& neighborInd :
	     BoardFieldNeighborCache::GetDirectNeighborIndicesFor(move.boardIndices)) {
		auto& neighborField = board[neighborInd];
		if (!neighborField->get_die() &&
		    ((neighborField->get_color_restriction().has_value() &&
		      neighborField->get_color_restriction().value() == move.die->get_color()) ||
		     (neighborField->get_value_restriction().has_value() &&
		      neighborField->get_value_restriction().value() == move.die->get_value()))) {
			return true;
		}
	}
 ...
 */
inline void insert_dtfm(move_by_index_c& goodMoves, move_by_index_c& backupMoves, Board& board,
 ...
 */
inline void filter_moves(Game& g, const move_c& allMoves, move_by_index_c& bestMoves,
                         move_by_index_c& backupMoves)
{
	...
	for (auto&& move : allMoves) {
		if (auto dtfm = move->get_placing_submove()) {
			...
			DieToFieldMove tmpDtfm{0, board[relocMove->from]->get_die(), relocMove->to,
			                       DTFM_MODE_DEFAULT};
			insert_dtfm(bestMoves, backupMoves, board, tmpDtfm, move);
		}
		else if (auto twoRelocMove = dynamic_cast<RelocateTwoDiceMove*>(move)) {
			DieToFieldMove tmpDtfm1{0, board[twoRelocMove->m1.from]->get_die(), twoRelocMove->m1.to,
			                        DTFM_MODE_DEFAULT};
			DieToFieldMove tmpDtfm2{0, board[twoRelocMove->m2.from]->get_die(), twoRelocMove->m2.to,
			                        DTFM_MODE_DEFAULT};
