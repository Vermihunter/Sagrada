...
class SagradaGameCMDLineParser : public SagradaCMDLineParser
{
  ...
  public:
    ...
    SagradaGameCMDLineParser(std::unique_ptr<std::ostream>& _logFileStream,
                             bool isTournamentParser,
                             SagradaCMDLineParser* _parent, Args&&... _args)
        : SagradaCMDLineParser(_logFileStream, std::forward<Args>(_args)...)
        , parent(_parent)
    {
        add_argument("-s")
        ...
        add_argument("-p")
            ...
            .action([&](std::string s) {
                ...
                ai_player_config_t config =
                    DefinedAIPlayers::construct_by_name(s);
                if (!config) {
