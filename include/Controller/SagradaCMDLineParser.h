...
{
	return DefaultLogSettings::logDir + "/" + Date::get_curr_date() + "-" + Date::get_curr_time() +
	       ".log";
}
...
inline /* [[noreturn]] */
    void
    exit_with_err_message(const std::string& errMessage, argparse::ArgumentParser& parser)
{
...
class SagradaCMDLineParser : public argparse::ArgumentParser {
  public:
	...
	SagradaCMDLineParser(std::unique_ptr<std::ostream>& _logFileStream, Args&&... _args)
	    : argparse::ArgumentParser(std::forward<Args>(_args)...), logFileStream(_logFileStream)
	{
		...
		add_argument("-ln").help("log file name or \"cout\" and \"cerr\" values are accepted");
	}
	...
	virtual void parse(int argc, char** argv)
	{
		...
		if (auto logfilename = present("-ln")) {
			...
			logFileStream       = std::make_unique<std::ofstream>(generate_log_file_name());
			logConfig.logStream = logFileStream.get();
		}
