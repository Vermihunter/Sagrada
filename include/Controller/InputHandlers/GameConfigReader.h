...
inline puoc_context_t get_puoc_ctx(json& j)
{
	...
	for (auto&& json_obj : j.at("puoc_contexts")) {
		puocConfig.push_back(
		    {json_obj.at("PuocId"),
		     PublicObjectiveCardContext{json_obj.at("SatisfactionValue").get<int>()}});
	}

	return std::make_unique<PublicObjectiveCardGameContext>(cardsPerGame, puocConfig);
}
...
{
	if (!j.is_array()) {
		throw std::invalid_argument{"WPC must be represented as an array of WPC names!"};
	}
	...
	wpc_c res;
	for (auto&& wpcTitle : j) {
		auto wpcIt = std::find_if(allWpc.begin(), allWpc.end(),
		                          [&](const wpc_t& wpc) { return wpc->title == wpcTitle; });
...
inline score_ctx_t get_score_ctx(json& j)
{
	...
	return std::make_unique<ScoreContext>(pointsPerUnusedFavorToken, minusPointsPerEmptyField,
	                                      pointsPerProcValue);
}
...
inline void process_json_config_file(const fs::path& path, game_ctx_builder_by_player_count_t& dest)
{
	...
	auto           playerRelatedInformation = jf.at("PlayerCountRelatedInformation")
	...
	static inline game_ctx_builder_by_player_count_t get_all_available_game_contexts()
	{
		...
		for (auto&& entry : fs::recursive_directory_iterator(gameConfigDir)) {
			...
			try {
				...
				std::cerr << "Invalid json config: " << entry.path() << " - " << e.what()
				          << std::endl;
			}
		}
