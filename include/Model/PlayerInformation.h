...
struct PlayerInformation {
	...
	PlayerInformation(const PlayerInformation& pi, DiceSupply& newDS, DiceSupply& originalDS)
	    : favorTokensLeft(pi.favorTokensLeft), ID(pi.ID), board(pi.board->clone(newDS, originalDS)),
	      name(pi.name), PrOC(pi.PrOC)
	{
	}
	...
	{
		return os << "{Name: " << obj.name << ", ID: " << obj.ID << ", PrOC: " << obj.PrOC << "}";
	}
};
