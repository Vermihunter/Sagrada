...
class RelocateMoveFiller {
  public:
	static void AddRelocateMove(move_c& dest, ID_t _playerId, size_t _ind, const BoardIndex& _from,
	                            const BoardIndex& _to, dtfm_mode_mask_t _modeMask)
	{
		dest.push_back(new RelocateDieMove(_playerId, _ind, _from, _to, _modeMask));
	}
};
 ...
 */
template <dtfm_mode_t _AttributeIgnorer> class RelocateDieIgnoreAttributeTC : public RelocatingTC {
  public:
	static constexpr dtfm_mode_mask_t mode = dtfm_mode_mask_t(_AttributeIgnorer);

	static_assert(mode == DTFM_MODE_IGNORE_COLOR || mode == DTFM_MODE_IGNORE_VALUE,
	...
	virtual void append_possible_moves(ToolCardAppendContext& ctx) override
	{
		...
		// relocMoves.end());
		append_all_relocate_moves<Move*, RelocateMoveFiller>(ctx.moveContainer, ctx, mode);
	}

	virtual tc_t clone() override { return std::make_unique<RelocateDieIgnoreAttributeTC>(*this); }
