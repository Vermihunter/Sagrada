...
enum class relocating_index_check_res_t { IND_OK, TWO_SAME_TO, TWO_SAME_FROM, TO_EQ_FROM };
enum class relocating_check_res_t { MOVE_OK, TO_EQ_FROM, NO_DIE, DTFM_BAD };
...
class RelocatingTC : public ToolCard {
  ...
  protected:
	...
	bool check_relocate_move_correct_softfail(const BoardIndex& from, const BoardIndex& to,
	                                          ID_t playerId, dtfm_mode_mask_t relocMoveMode,
	                                          Game& g);
	void check_relocate_move_correct_hardfail(relocate_move_t relocMove, Game& g);
	...
	bool check_indices_correct_softfail(const RelocateDieMove& mv1, const RelocateDieMove& mv2);
	void check_indices_correct_hardfail(const RelocateDieMove& mv1, const RelocateDieMove& mv2);

	template <typename T, typename ContainerFiller>
	void append_all_relocate_moves(std::vector<T>& destContainer, ToolCardAppendContext& ctx,
		...
		for (auto&& row : board->get_ctx()->data) {
			for (auto&& field : row) {
				...
				for (auto&& emptyFieldInd : board->get_empty_fields()) {
					if (!check_relocate_move_correct_softfail(ind, emptyFieldInd, ctx.playerId,
					                                          mode, ctx.g)) {
						continue;
					}
					ContainerFiller::AddRelocateMove(destContainer, ctx.playerId, ctx.toolCardInd,
					                                 ind, emptyFieldInd, mode);
  ...
  private:
	relocating_check_res_t check_relocate_move_correct(const BoardIndex& from, const BoardIndex& to,
	                                                   ID_t             playerId,
	                                                   dtfm_mode_mask_t relocMoveMode, Game& g);

	relocating_index_check_res_t check_indices_correct(const RelocateDieMove& mv1,
