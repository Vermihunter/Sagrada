...
class PutDieWithoutNeighborTC : public ToolCard {
  public:
	...
	virtual tc_t clone() override { return std::make_unique<PutDieWithoutNeighborTC>(*this); }
	...
	struct Move : ToolCardMove {
		...
		virtual move_t clone(const DiceSupply& from, const DiceSupply& to) override
		{
			return new PutDieWithoutNeighborTC::Move(
			    playerId, ind,
			    DieToFieldMove{playerId, get_corresponding_die(bm.die, from, to), bm.boardIndices,
			                   bm.mode});
		}
