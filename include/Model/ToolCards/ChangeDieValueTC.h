...
class ChangeDieValueTC : public ToolCard
{
  public:
    ...
    {
        return std::make_unique<ChangeDieValueTC>(*this);
    }
     ...
     * value changers */
    void process_hypothetical_die_with_val(ToolCardAppendContext& ctx, Die* die,
                                           int changeVal);

    struct Move : ToolCardMove
    {
        Move(ID_t _playerId, size_t _ind, Die* _die, int _changeVal,
             const BoardIndex& boardInd)
            : ToolCardMove(_playerId, _ind)
            , dtfm(_playerId, _die, boardInd, DTFM_MODE_DEFAULT)
        ...
        {
            return new ChangeDieValueTC::Move(
                playerId, ind, get_corresponding_die(dtfm.die, from, to),
                changeVal, dtfm.boardIndices);
        }

        static tool_card_move_t from_context(ToolCardAppendContext& ctx,
        ...
        {
            ChangeDieValueTC::Move* otherTCMove =
                dynamic_cast<ChangeDieValueTC::Move*>(otherMove);
            if (!otherTCMove) {
                return false;
            }

            return dtfm.equals_to_dtfm(otherTCMove->dtfm) &&
                   changeVal == otherTCMove->changeVal;
        }
  ...
  private:
    ...
    /** Use implementation */
    void append_moves_for_die(ToolCardAppendContext& ctx, Die* die,
                              int changeVal);
