...
class RerollAllDiceTC : public ToolCard {
  public:
	...
	virtual tc_t clone() override { return std::make_unique<RerollAllDiceTC>(*this); }
	...
	struct Move : ToolCardMove {
		...
		virtual bool equals(const move_t& otherMove) override
		{
			...
			if (!otherRerollingMove ||
			    originalValues.size() != otherRerollingMove->originalValues.size()) {
				return false;
			}

			for (size_t i = 0; i < originalValues.size(); ++i) {
				if (*originalValues[i].first != *otherRerollingMove->originalValues[i].first ||
				    originalValues[i].second != otherRerollingMove->originalValues[i].second) {
					return false;
				}
			}
