...
struct ToolCardAppendContext {
	ToolCardAppendContext(move_c& _mc, Game& _g, size_t tcInd, ID_t pId, tool_card_move_t _prevMove)
	    : moveContainer(_mc), g(_g), toolCardInd(tcInd), playerId(pId), prevMove(_prevMove)
	{
	}
...
class ToolCard {
  ...
  protected:
	...
	static Derived* to_concrete_type(tool_card_move_t& m)
	{
		...
		if (!concreteType) {
			throw std::runtime_error{"Invalid tool card move type passed to use!"};
		}
