...
class ReplaceDieWithRandomFromBagTC : public ToolCard {
  public:
	...
	struct DieToChangeMove : ToolCardMove {
		DieToChangeMove(ID_t _playerId, size_t _ind, Die* die, bool _wasPlaceable = false)
		    : ToolCardMove(_playerId, _ind), currentlyUsedDie(die), dieInBag(nullptr),
		...
		{
			auto mv = new ReplaceDieWithRandomFromBagTC::DieToChangeMove(playerId, ind, nullptr,
			                                                             wasPlaceable);
			mv->currentlyUsedDie = get_corresponding_die(currentlyUsedDie, from, to);
			mv->dieInBag         = (dieInBag) ? get_corresponding_die(dieInBag, from, to) : nullptr;
			return mv;
		}
		...
		{
			ReplaceDieWithRandomFromBagTC::DieToChangeMove* otherDieSwappingMove =
			    dynamic_cast<ReplaceDieWithRandomFromBagTC::DieToChangeMove*>(otherMove);
			if (!otherDieSwappingMove) {
				return false;
			}

			return *currentlyUsedDie == *otherDieSwappingMove->currentlyUsedDie &&
			       ((dieInBag == nullptr && otherDieSwappingMove->dieInBag == nullptr) ||
			        (dieInBag != nullptr && otherDieSwappingMove->dieInBag != nullptr &&
	...
	struct ValueChangerAndPlacerMove : ToolCardMove {
		ValueChangerAndPlacerMove(ID_t _playerId, size_t _ind, value_t val, value_t _prevVal,
		                          Die* _dieToPlace, const BoardIndex& _placeInd)
		    : ToolCardMove(_playerId, _ind), newVal(val), prevVal(_prevVal),
		...
		virtual void print(std::ostream& os) const override
		{
			os << "ReplaceDieWithRandomFromBagToolCard::ValueChoosingMove: "
			      "[playerID: "
			   << playerId << ", die value changed from " << size_t(prevVal) << " to "
			   << size_t(newVal) << " - placing " << move_t(&placingMove) << "]";
		}

		virtual move_t clone(const DiceSupply& from, const DiceSupply& to) override
		{
			Die* correspondingDie = get_corresponding_die(placingMove.die, from, to);
			return new ReplaceDieWithRandomFromBagTC::ValueChangerAndPlacerMove(
			    playerId, ind, newVal, prevVal, correspondingDie, placingMove.boardIndices);
		}

		virtual die_to_field_move_t get_placing_submove() override { return &placingMove; }
		...
		virtual bool equals(const move_t& otherMove) override
		{
			ReplaceDieWithRandomFromBagTC::ValueChangerAndPlacerMove* otherDiePlacingMove =
			    dynamic_cast<ReplaceDieWithRandomFromBagTC::ValueChangerAndPlacerMove*>(otherMove);
  ...
  private:
	...
	void cache_placing_moves_for(Die* die, value_t diePrevVal, ToolCardAppendContext& ctx);
