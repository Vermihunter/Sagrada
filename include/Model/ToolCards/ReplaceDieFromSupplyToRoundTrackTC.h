...
class ReplaceDieFromSupplyToRoundTrackTC : public ToolCard {
  public:
	...
	struct Move : ToolCardMove {
		Move(ID_t _playerId, size_t _ind, Die* dieToReplace, Die* dieToReplaceWith,
		     const DieToFieldMove& move)
		    : ToolCardMove(_playerId, _ind), dieInSupply(dieToReplace),
		      dieOnRoundTrack(dieToReplaceWith), placingMove(move)
		{
		}

		virtual void print(std::ostream& os) const override
		{
			os << "ReplaceDieFromSupplyToRoundTrackToolCardMove: [playerID: " << playerId
			   << ", die from supply: " << dieInSupply
		...
		virtual move_t clone(const DiceSupply& from, const DiceSupply& to) override
		{
			auto mv = new ReplaceDieFromSupplyToRoundTrackTC::Move(
			    ...
			    DieToFieldMove{playerId, get_corresponding_die(placingMove.die, from, to),
			                   placingMove.boardIndices, placingMove.mode});
			mv->dieInSupply     = get_corresponding_die(dieInSupply, from, to);
			mv->dieOnRoundTrack = get_corresponding_die(dieOnRoundTrack, from, to);
			return mv;
		}
		...
		virtual bool equals(const move_t& otherMove) override
		{
			ReplaceDieFromSupplyToRoundTrackTC::Move* otherReplacingMove =
			    dynamic_cast<ReplaceDieFromSupplyToRoundTrackTC::Move*>(otherMove);
  ...
  private:
	...
	void append_moves_for(Die* currentRoundDie, Die* roundTrackDie, ToolCardAppendContext& ctx);
};
