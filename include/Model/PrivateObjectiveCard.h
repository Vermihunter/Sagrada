...
class PrivateObjectiveCard : ICloneable<PrivateObjectiveCard> {
  public:
	constexpr static unsigned int totalCardCount = (unsigned int)(color_t::COUNT);
	...
	PrivateObjectiveCard(const PrivateObjectiveCard& other) { this->color = other.color; }
	...
	{
		if (t == color_t::COUNT) return nullptr;

		return std::make_unique<PrivateObjectiveCard>(t);
	}

	/** Debug */
	friend std::ostream& operator<<(std::ostream& os, std::shared_ptr<PrivateObjectiveCard>& proc)
	{
