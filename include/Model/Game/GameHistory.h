...
struct GameHistory {
	...
	GameHistory(const GameHistory& other, const DiceSupply& from, const DiceSupply& to)
	{
		...
		for (auto&& m : other.moveStack) {
			...
			if (prevInfo && prevInfo->move->t == Move::type::TOOL_CARD) {
				auto tcMoveInfo =
				    ((tool_card_move_t)(prevInfo->move))->consists_of_multiple_submoves();
				if (!tcMoveInfo.isSingleMoveToolCard && tcMoveInfo.followUpMoveExists) {
					prevToolCardMove = ((tool_card_move_t)(prevInfo->move));
				}
			}

			moveStack.push_back(MoveInfo{.move                 = m.move->clone(from, to),
