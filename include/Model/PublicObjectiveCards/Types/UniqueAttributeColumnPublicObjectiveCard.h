...
class UniqueAttributeColumnPublicObjectiveCard : public PublicObjectiveCard {
	using col_info_t = PublicObjectiveCardRowAndColumnEvaluatingInformation<Attribute>;
  ...
  public:
	...
	UniqueAttributeColumnPublicObjectiveCard(const UniqueAttributeColumnPublicObjectiveCard& other)
	...
	virtual puoc_pattern_t evaluate(OverallBoardState& overallState, board_t& board) override
	{
			...
			for (auto&& field : row) {
				...
				if (!die) {
					...
					if (fieldRestriciton.has_value()) {
						attributeCountByColumn[ind.col].emptyFieldRestrictions.insert(
		...
		for (auto&& columnAttributes : attributeCountByColumn) {
			puoc_eval_pattern_state state =
			    column_pattern_match(columnAttributes, board->get_ctx()->config);
			...
			if (state == puoc_eval_pattern_state::MATCHED) {
				...
				int diceToComplete = dice_to_complete_column_pattern(overallState, columnAttributes,
	...
	{
		return std::make_unique<UniqueAttributeColumnPublicObjectiveCard<Attribute>>(*this);
	}

  private:
	int dice_to_complete_column_pattern(OverallBoardState& overallState,
	                                    col_info_t& columnAttributes, const board_config_t& config)
	{
	                                             ...
	                                             const board_config_t& config)
	{
		 ...
		 * row
		 */
		return (columnAttributes.attrIndices.back() == 0) ? puoc_eval_pattern_state::MATCHED
