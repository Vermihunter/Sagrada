...
class UniqueAttributeRowPublicObjectiveCard : public PublicObjectiveCard {
  public:
	using row_info_t = PublicObjectiveCardRowAndColumnEvaluatingInformation<Attribute>;
	...
	UniqueAttributeRowPublicObjectiveCard(const UniqueAttributeRowPublicObjectiveCard& other)
	...
	virtual puoc_pattern_t evaluate(OverallBoardState& overallState, board_t& board) override
	{
		...
		std::vector<bool> unmatchableRows(board->get_ctx()->config->height, false);
		...
		for (auto&& row : board->get_ctx()->data) {
			...
			for (auto&& field : row) {
				...
				if (!die) {
					auto fieldRestriciton = field->template get_attr_restriction<Attribute>();
					// Remembering restrictions for pattern completability
					// checking
					if (fieldRestriciton.has_value()) {
						attributeCountByRow[ind.row].emptyFieldRestrictions.insert(
		...
		for (auto&& rowAttributes : attributeCountByRow) {
			...
			// or may not be completed
				...
				int diceToComplete = dice_to_complete_row_pattern(overallState, rowAttributes,
				                                                  board->get_ctx()->config);
				if (diceToComplete > 0) {
					puocResultState.completableSubPatterns.push_back({diceToComplete});
				}
				else {
	...
	{
		return std::make_unique<UniqueAttributeRowPublicObjectiveCard<Attribute>>(*this);
	}
  ...
  private:
	                                          ...
	                                          const board_config_t& config)
	{
		 ...
		 * row
		 */
		return (rowAttributes.attrIndices.back() == 0) ? puoc_eval_pattern_state::MATCHED
		                                               : puoc_eval_pattern_state::NONE;
	}

	int dice_to_complete_row_pattern(OverallBoardState& overallState, row_info_t& rowAttributes,
	                                 const board_config_t& config)
	{
