...
class DiagonalConnectingPublicObjectiveCard : public PublicObjectiveCard
{
  public:
    ...
    DiagonalConnectingPublicObjectiveCard(
        const DiagonalConnectingPublicObjectiveCard& other)
                                    ...
                                    board_t& board) override
    {
        ...
        for (auto&& row : board->get_ctx()->data) {
            for (auto&& field : row) {
                ...
                for (
                    auto&& neighborInd :
                    BoardFieldNeighborCache::GetDiagonalNeighborIndicesFor(
                    ...
                    if (!neighborDie)
                        continue;
                    Attribute neighborAttribute =
                        Die::get_attr<Attribute>(neighborDie);
                    ...
                    // put on the empty field
                        ...
                        if constexpr (std::is_same_v<Attribute, color_t>) {
                            anyCorrectPlaceable =
                                placeabilityChecker->any_with_color(
                                    *board, ind, neighborAttribute);
                        }
                        /*
    ...
    virtual std::unique_ptr<PublicObjectiveCard> clone() override
    {
        return std::make_unique<
            DiagonalConnectingPublicObjectiveCard<Attribute>>(*this);
    }
};
