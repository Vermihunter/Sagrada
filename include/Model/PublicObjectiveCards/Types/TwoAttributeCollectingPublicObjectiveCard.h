...
class TwoAttributeCollectingPublicObjectiveCard : public PublicObjectiveCard {
  public:
	TwoAttributeCollectingPublicObjectiveCard(concrete_puoc_context_t ctx, Attribute _attr1,
	                                          Attribute _attr2)
	...
	TwoAttributeCollectingPublicObjectiveCard(
	    ...
	    : PublicObjectiveCard(other.ctx->clone()), attr1(other.attr1), attr2(other.attr2)
	{
	}
	...
	virtual puoc_pattern_t evaluate(OverallBoardState& overallState, board_t& board) override
	{
		...
		int totalFieldCount = board->get_ctx()->config->height * board->get_ctx()->config->width;
		int attr1PlaceableToEmpty = totalFieldCount;
		int attr2PlaceableToEmpty = totalFieldCount;

		std::vector<std::pair<bool, bool>> placeableAttributeCache(totalFieldCount,
		                                                           std::pair{true, true});
			...
			for (auto&& field : row) {
				...
				if (fieldAttribute == attr1) {
					...
					for (auto&& directNeighborInd :
						...
						if (placeableAttributeCache[directNeighborInd.col * directNeighborInd.row]
						        .first) {
							--attr1PlaceableToEmpty;
							placeableAttributeCache[directNeighborInd.col * directNeighborInd.row]
							    .first = false;
						}
					}
				}
		...
		puocResultState.totalEarnedPoints = minAttrPresence * ctx->get_satisfaction_value();
		...
		int remainingMinpresent = std::min(attr1PlaceableToEmpty, attr2PlaceableToEmpty);
		if (remainingMinpresent > 0) {
			puocResultState.completableSubPatterns.insert(
			    puocResultState.completableSubPatterns.end(), remainingMinpresent,
	...
	{
		return std::make_unique<TwoAttributeCollectingPublicObjectiveCard<Attribute>>(*this);
	}
