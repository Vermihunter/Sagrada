...
using puoc_config_c =
    std::vector<std::pair<puoc_etype, PublicObjectiveCardContext>>;
...
{
    PublicObjectiveCardGameContext(int _cardsPerGame,
                                   const puoc_config_c& _selectedCards)
    ...
    virtual bool contains(puoc_etype type) override
    {
        auto cardIt = std::find_if(
            selectedCards.begin(), selectedCards.end(),
