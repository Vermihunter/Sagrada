...
class Board {
  public:
	...
	auto from_ctx(board_ctx_t ctx) { return std::make_unique<Board>(std::move(ctx)); }
	 ...
	 */
	auto& operator[](const BoardIndex& index) const { return fastctx->data[index.row][index.col]; }
