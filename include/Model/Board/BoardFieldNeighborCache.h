...
class BoardFieldNeighborCache
{
  ...
  private:
    ...
    BoardFieldNeighborCache()
        : cachedNeighbors(rowCount, std::vector<AllNeighbors>(colCount))
    {
        for (int i = 0; i < rowCount; ++i) {
            for (int j = 0; j < colCount; ++j) {
                LoadAllIndices(
                    i, j, relativeDirectIndices,
                    cachedNeighbors[i][j].absolutDirectNeighborIndices);
                LoadAllIndices(
                    i, j, relativeDiagonalIndices,
        ...
        std::vector<BoardIndex>& dest)
    {
        for (auto&& relativeIndex : relativeIndices) {
            ...
            if (IsCorrectInd(newRowInd, newColInd, rowCount, colCount)) {
                dest.push_back(
                    BoardIndex {size_t(newRowInd), size_t(newColInd)});
            }
        }
    }
