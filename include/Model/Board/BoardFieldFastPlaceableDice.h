...
template <>
struct BitmaskGenerator<Value>
{
    static all_dice_t get_have_bitmask(size_t valueInd)
    {
        return (value_one_bit_mask << valueInd);
    }
...
struct FastPlaceableDice
{
    ...
    {
        return any_having_attribute<Color>(color);
    }

    bool any_having_value(value_t value)
    {
        return any_having_attribute<Value>(value);
    }
    ...
    template <typename A>
    bool any_having_attribute(A::TYPE attr)
    {
    ...
    template <typename A>
    void disallow_attribute(A::TYPE attr)
    {
        size_t attrInd = size_t(attr);
