...
class BoardField {
  ...
  public:
	...
	template <typename A> constexpr auto get_attr_restriction()
	{
		if constexpr (std::is_same_v<A, value_t>) return get_value_restriction();
		if constexpr (std::is_same_v<A, color_t>) return get_color_restriction();
	}

	auto& get_color_restriction() const { return restrictions->colorRestriction; }
	auto& get_value_restriction() const { return restrictions->valueRestriction; }
