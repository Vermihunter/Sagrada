...
struct BoardIndex {
	...
	template <std::size_t I> using tuple_element = index_t&;

	template <size_t index> constexpr index_t& get()
	{
		if constexpr (index == 0) return row;
		if constexpr (index == 1) return col;
	}
...
template <> struct std::hash<BoardIndex> {
