...
class RectangularBoardTypeNeighborProcessor : public BoardTypeNeighborProcessor
{
  ...
  public:
    virtual bool is_correct_index(const BoardIndex& ind,
                                  const board_config_t& config) override
    {
        return ind.row < config->height && ind.col < config->width;
    }

    virtual bool are_direct_neighbors(const BoardIndex& ind1,
                                      const BoardIndex& ind2) override
    {
        return get_ind_abs(ind1, ind2) == 1;
    }

    virtual bool are_diagonal_neighbors(const BoardIndex& ind1,
                                        const BoardIndex& ind2) override
    {
        return get_ind_abs(ind1, ind2) == 2;
    }

    virtual bool are_neighbors(const BoardIndex& ind1,
                               const BoardIndex& ind2) override
    {
        return are_direct_neighbors(ind1, ind2) ||
               are_diagonal_neighbors(ind1, ind2);
    }
  ...
  private:
    ...
    template <size_t N>
    std::vector<BoardIndex> get_neighbor_indices_impl(
        const BoardIndex& ind, const board_config_t& config,
        const std::array<std::pair<int, int>, N>& relativeIndContainer)
    {
        ...
        for (auto&& [iNeighborRow, iNeighborCol] : relativeIndContainer) {
            ...
            if (iAbsoluteNeighborCol < 0 || iAbsoluteNeighborRow < 0)
                continue;

            BoardIndex newInd {size_t(iAbsoluteNeighborRow),
                               size_t(iAbsoluteNeighborCol)};
            if (!is_correct_index(newInd, config))
                continue;
...
struct RectangleBoardDataInputHandler : BoardDataInputHandler
{
    ...
    virtual BoardRowParsingFeedBack handle_line(
        const std::string& line) override
    {
        ...
        ++currLine;
        return BoardRowParsingFeedBack {
            lineCorrect, /* lastLine = */ (currLine == config->height)};
    }
...
struct RectangleBoardFieldPlaceableOperator : BoardFieldPlaceableOperator
{
    ...
    virtual bool any_with_color(Board& board, const BoardIndex& atInd,
                                color_t col) override
    {
        return any_with_attribute(board, atInd, col);
    }

    virtual bool any_with_value(Board& board, const BoardIndex& atInd,
                                value_t val) override
    {
  ...
  private:
    ...
    bool any_with_attribute(Board& board, const BoardIndex& ind, Attribute attr)
    {
        ...
        std::optional<Attribute> attr_restriction =
            field->get_attr_restriction<Attribute>();
    ...
    template <typename Attribute>
    bool check_for_colliding(Board& board, const BoardIndex& ind,
                             Attribute attr)
    {
        for (auto&& neighborInd :
             BoardFieldNeighborCache::GetDirectNeighborIndicesFor(ind)) {
            ...
            // one
            if (attr == Die::get_attr<Attribute>(neighborDie))
                return false;
        }
...
class RectangularBoardType : public BoardType
{
  ...
  public:
    virtual std::unique_ptr<BoardDataInputHandler> construct_input_handler(
        board_data_t& boardData, board_config_t config) override
    {
        return std::make_unique<RectangleBoardDataInputHandler>(boardData,
                                                                config);
    }
    ...
    virtual const std::vector<BoardIndex>& get_all_indices() const
    {
        return allIndices;
    }
