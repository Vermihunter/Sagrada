...
struct RelocateTwoDiceMove : ToolCardMove
{
    RelocateTwoDiceMove(const RelocateDieMove& _m1, const RelocateDieMove& _m2)
        : ToolCardMove(_m1.playerId, _m1.ind)
        , m1(_m1)
        , m2(_m2)
        , hasSecondRelocateMove(true)
    {
    }

    RelocateTwoDiceMove(const RelocateDieMove& singleMove)
        : ToolCardMove(singleMove.playerId, singleMove.ind)
        , m1(singleMove)
        , m2(singleMove)
    ...
    {
        RelocateTwoDiceMove* otherRelocatingMove =
            dynamic_cast<RelocateTwoDiceMove*>(otherMove);
        if (!otherRelocatingMove) {
            return false;
        }

        return m1.equals_to_relocate_move(otherRelocatingMove->m1) &&
               ((!hasSecondRelocateMove &&
                 !otherRelocatingMove->hasSecondRelocateMove) ||
                (hasSecondRelocateMove &&
                 otherRelocatingMove->hasSecondRelocateMove &&
