...
{
    RelocateDieMove(ID_t _playerId, size_t _ind, const BoardIndex& _from,
                    const BoardIndex& _to, dtfm_mode_mask_t _modeMask = 0)
    ...
    {
        os << "Relocating Tool Card Move: [PlayerID: " << playerId
           << ", from: " << from << ", to: " << to << "]";
    }
    ...
    {
        RelocateDieMove* otherRelocatingMove =
            dynamic_cast<RelocateDieMove*>(otherMove);
        return (otherRelocatingMove != nullptr
                    ? equals_to_relocate_move(*otherRelocatingMove)
                    : false);
    }
