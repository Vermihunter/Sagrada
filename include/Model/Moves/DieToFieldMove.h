...
struct DieToFieldMove : Move
{
    DieToFieldMove(ID_t _playerId, Die* _die, const BoardIndex& _boardIndices,
        ...
        : Move(type::DIE_TO_FIELD_MOVE, _playerId)
        , die(_die)
        , boardIndices(_boardIndices)
        , mode(_modeMask)
    {
    }
    ...
    static die_to_field_move_t create(ID_t playerId, Die* die,
                                      const BoardIndex& boardIndices,
    ...
    {
        return DieToFieldMove::create(
            playerId, get_corresponding_die(die, from, to), boardIndices, mode);
    }

    virtual void print(std::ostream& os) const override
    {
        os << "DieToFieldMove: [playerID: " << playerId
           << ", to: " << boardIndices << ", " << die << ", ";
    ...
    {
        die_to_field_move_t otherDtfm =
            dynamic_cast<DieToFieldMove*>(otherMove);
        return (otherDtfm != nullptr ? equals_to_dtfm(*otherDtfm) : false);
    }

    bool equals_to_dtfm(const DieToFieldMove& otherDtfm)
    {
        return *die == *otherDtfm.die &&
               boardIndices == otherDtfm.boardIndices && mode == otherDtfm.mode;
    }
