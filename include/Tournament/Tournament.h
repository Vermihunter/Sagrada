...
class Tournament {
  public:
	 ...
	 */
	tournament_game_info_t launch_game(int seed, size_t gameNumber, ai_player_config_pc& aiConfigs);

	Tournament(tournament_config_t _config, TournamentStatistics& _destStatistics);
  ...
  private:
	...
	std::vector<tournament_game_info_t> launch_games(int seed, size_t gameGroupCount,
	                                                 std::vector<ai_player_config_pc>& aiConfigs);

	game_result_t update_statistics(tournament_game_info_t& controller);
	void print_results(tournament_game_info_t& gameInfo, game_result_t& gameResult, int seed);
};
