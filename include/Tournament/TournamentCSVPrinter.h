...
using tournament_csv_printer_config_t =
    std::unique_ptr<TournamentCSVPrinterConfig>;
...
struct TournamentCSVPrinterConfig
{
    static tournament_csv_printer_config_t create_default(
        const std::string& _outputDir, const std::vector<std::string>& _columns)
    {
        return std::make_unique<TournamentCSVPrinterConfig>(',', _outputDir,
                                                            _columns);
    }
...
class TournamentCSVPrinter : public ITournamentPrinter
{
  public:
        ...
        , overallGameStatisticsOutputStream((const char*)(nullptr))
    {
        ...
        gameResultOutputStream.open(fs::path(config->outputDir) /
                                    fs::path("game_results.csv"));
        overallGameStatisticsOutputStream.open(fs::path(config->outputDir) /
                                               fs::path("game_statistics.csv"));
        playerStatistcsStream.open(fs::path(config->outputDir) /
                                   fs::path("player_statistics.csv"));
        ...
        if (tournamentConfig.saveMoveStatistics) {
            ...
            writeToCSV(*moveStatisticsOutputStream, "GameNumber", "RoundNumber",
                       "PlayerType", "DTFM_Count", "TC_1_MoveCount",
                       "TC_2_MoveCount", "TC_3_MoveCount", "TotalMoveCount",
                       "FilteredTotalMoveCount");
        }
                                       ...
                                       AI_PlayerConfig* playerConfig) override
    {
        ...
        writeToCSV(*moveStatisticsOutputStream, gameNumber,
                   g.get_curr_state()->roundNumber,
                   std::quoted(playerConfig->get_name()), dtfmCount,
                   tcMoveCount[0], tcMoveCount[1], tcMoveCount[2], moves.size(),
    ...
    virtual void print_overall_statistics(
        const TournamentStatistics& statistics)
    {
        // player statistics
        writeToCSV(playerStatistcsStream, "AI type", "Win count",
                   "Placement points", "Average points",
                   "Total move time in ms", "Total move count",
                   "Average move time in ms");
        for (auto&& [playerConfig, playerResults] :
             statistics.playerStatistics) {
            writeToCSV(
                playerStatistcsStream, "\"" + playerConfig->get_name() + "\"",
        ...
        auto wilsonInterval =
            statistics.playerStatistics.at(winnerPlayer)
                .get_wilson_score_interval_with_98_confidence();
        writeToCSV(overallGameStatisticsOutputStream,
                   "Wilson interval lower bound in %",
                   "Wilson interval upper bound in %",
                   "Wilson interval confidence level", "Winner player type");
        overallGameStatisticsOutputStream << std::setprecision(3);
        writeToCSV(overallGameStatisticsOutputStream,
                   wilsonInterval.interval_low * 100,
                   wilsonInterval.interval_high * 100,
                   wilsonInterval.confidence,
  ...
  private:
    ...
    template <typename T>
    void writeToCSV(std::ofstream& file, const T& last)
    {
        file << last << std::endl;
    }
