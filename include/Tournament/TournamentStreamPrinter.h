...
std::ostream& operator<<(std::ostream& os, const MiddleAlignManip& manip)
{
	...
	return os << std::string(leftPadding, ' ') << manip.text << std::string(rightPadding, ' ');
}
...
class TournamentStreamPrinter : public ITournamentPrinter {
  public:
	TournamentStreamPrinter(const TournamentConfig& _tournamentConfig, std::ostream& _os)
	    : ITournamentPrinter(_tournamentConfig), os(_os), gameCount(1), rowWidth(0)
	{
	                               ...
	                               int usedSeed) override
	{
		...
		for (size_t i = 0; i < tournamentConfig.gameConfig->players.size(); ++i) {
			...
			gameResultColumns[2 + i * 2 + 1] = std::to_string(pointByPlayerIt->second);
	...
	virtual void print_overall_statistics(const TournamentStatistics& statistics) override
	{
		// player statistics
		...
		std::vector<ColumnInformation> playerStatisticsColumns{
		    ColumnInformation{
		        .width = aiTypeColumnWidth, .colorHighlighter = YELLOW, .header = "AI type"},
		    ColumnInformation{.width = 11, .colorHighlighter = RED, .header = "Win count"},
		    ColumnInformation{.width = 16, .colorHighlighter = BLUE, .header = "Average points"},
		    ColumnInformation{.width            = tmpRowWidth - 11 - aiTypeColumnWidth - 16,
		...
		for (auto&& [aiConfig, aiStat] : statistics.playerStatistics) {
			os << '#' << playerStatisticsColumns[0].colorHighlighter
			   << std::setw(playerStatisticsColumns[0].width - 1) << aiConfig->get_name()
			   << defaultHighlighter << " #" << playerStatisticsColumns[1].colorHighlighter
			   << std::setw(playerStatisticsColumns[1].width - 1) << aiStat.winCount
			   << defaultHighlighter << " #" << playerStatisticsColumns[2].colorHighlighter
			   << std::setw(playerStatisticsColumns[2].width - 1)
			   << double(aiStat.totalReceivedPoints) / double(statistics.gameResults.size())
			   << defaultHighlighter << " #" << playerStatisticsColumns[3].colorHighlighter
			   << std::setw(playerStatisticsColumns[3].width - 1)
			   << double(aiStat.totalMoveTimeInMs) / double(aiStat.totalMoveCount)
		...
		os << "#" << gameStatisticsColumns[0].colorHighlighter
		   << middleAlign(gameStatisticsColumns[0].width, totalWinner->get_name())
		   << defaultHighlighter << "#" << std::endl;

		WilsonScoreInterval wilsonIterval = statistics.playerStatistics.at(totalWinner)
		                                        .get_wilson_score_interval_with_98_confidence();

		std::ostringstream oss;
		oss << std::setprecision(3);
		os << "# " << RED;
		oss << totalWinner->get_name() << " player wins " << wilsonIterval.interval_low * 100
		    << "% - " << wilsonIterval.interval_high * 100
		    << "% of games with 98% confidence level";
		os << middleAlign(rowWidth - 3, oss.str()) << defaultHighlighter << "#" << std::endl;
  ...
  private:
	...
	void print_header_row(const std::vector<ColumnInformation>& headerRow, const std::string& title)
	{
		os << std::string(rowWidth, '#') << std::endl;
		os << "# " << CYAN << middleAlign(rowWidth - 3, title) << defaultHighlighter << "#"
		   << std::endl;
		os << std::string(rowWidth, '#') << std::endl;

		os << '#';
		for (auto&& column : headerRow) {
			os << column.colorHighlighter << middleAlign(column.width, column.header)
			   << defaultHighlighter << "#";
		}
	...
	void print_header(const game_result_t& result, int usedSeed)
	{
		columns.push_back(ColumnInformation{
		    .width = 12, .colorHighlighter = gameCountHighlighter, .header = "Game count"});
		columns.push_back(ColumnInformation{
		    .width            = size_t(std::max(
		                   4, std::max(get_number_of_digits(tournamentConfig.gameConfig->randomSeed) + 2,
		                               get_number_of_digits(tournamentConfig.gameConfig->randomSeed +
		                                                    tournamentConfig.gameCount)) +
		                          2)),
		    .colorHighlighter = seedHighlighter,
		    .header           = "Seed"});

		gameResultColumns.resize(3 + 2 * tournamentConfig.gameConfig->players.size());
		size_t ind   = 1;
		maxNameWidth = 0;
		...
		for (auto&& aiConfig : tournamentConfig.gameConfig->players) {
			...
			columns.push_back(
			    ColumnInformation{.width = std::max(7 + 2 + indWidth, AI_playerNameWidths[ind - 1]),
			                      .colorHighlighter = playerTypeHighlighter,
			                      .header           = "Player " + indStr});
			columns.push_back(ColumnInformation{.width            = 6 + 2 + indWidth,
			                                    .colorHighlighter = playerScoreHighlighter,
			                                    .header           = "Score " + indStr});
			++ind;
		}

		columns.push_back(ColumnInformation{.width = std::max(size_t(9 + 2), maxNameWidth),
	...
	void print_centered(const std::string& str, const ColumnInformation& columnInfo)
	{
	...
	int get_number_of_digits(size_t number) { return static_cast<int>(log10(number)) + 1; }
};
