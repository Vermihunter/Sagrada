...
struct DefaultGameCreator {
	static std::unique_ptr<Game> create(size_t playerCount, rnd_t rnd)
	{
		...
		return std::make_unique<Game>(get_default_context(playerCount, std::move(rnd)),
	...
	static auto get_default_player_count_related_config()
	{
		return std::vector<PlayerCountRelatedGameInformation>{
		    PlayerCountRelatedGameInformation{.playerCount = 2, .diceCount = 5},
		    PlayerCountRelatedGameInformation{.playerCount = 3, .diceCount = 7},
		    PlayerCountRelatedGameInformation{.playerCount = 4, .diceCount = 9}};
	}
	...
	static game_context_t get_default_context(size_t playerCount, rnd_t rnd)
	{
		...
		return GameContextBuilder::create(get_default_player_count_related_config(),
	...
	static puoc_context_t get_default_puoc_context()
	{
		puoc_config_c puocConfig{
		    {puoc_etype::UNIQUE_VALUE_ROW, 5},        {puoc_etype::UNIQUE_COLOR_ROW, 6},
		    {puoc_etype::UNIQUE_VALUE_COLUMN, 4},     {puoc_etype::UNIQUE_COLOR_COLUMN, 5},
		    {puoc_etype::ONE_AND_TWO_COLLECTING, 2},  {puoc_etype::THREE_AND_FOUR_COLLECTING, 2},
		    {puoc_etype::FIVE_AND_SIX_COLLECTING, 2}, {puoc_etype::ALL_VALUE_COLLECTING, 5},
		    {puoc_etype::ALL_COLOR_COLLECTING, 4},    {puoc_etype::DIAGONAL_CONNECTING, 1}};
