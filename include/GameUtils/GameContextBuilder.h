...
class GameContextBuilder {
  public:
	 ...
	 */
	static game_context_builder_t
	create(const std::vector<PlayerCountRelatedGameInformation>& playerCountRelatedConfigs,
  ...
  private:
	 ...
	 * card color
	 */
	color_t get_unused_proc_color(std::vector<size_t>& randomizedProcIndices, size_t& startingIndex,
	...
	GameContextBuilder& add_concrete_card(CardType type, std::vector<CardType>& dest,
	 ...
	 */
	template <typename CardType>
	std::vector<CardType> add_missing_cards(std::vector<CardType>&     cardBuildingContainer,
	                                        CardGameContext<CardType>& cardContext, rnd_t& rnd)
	{
		...
		auto tmpCardTypeIndices = rnd->generate_random_id_order(cardContext.selected_card_count());
